...
ubiquitous in a vast array of application areas.  \djde{refs? maybe
  not necessary.} Very often, researchers studying ODE models are
highly skilled in dynamical systems theory, but lack experience
...
solutions of the ODEs (so consider all possible parameter values and
initial states) and identify the solution curve that is closest to
the observed data.  Occasionally, this might actually turn out to be
straightforward.  But more often than not, a variety of factors make
fitting an ODE to data a challenging problem.  In particular:
\begin{itemize}
...
\item Different measures of ``goodness of fit'' may identify different
  solutions as the ``best fit''.
\item Multiple solutions, associated with different regions of
  parameter space, may actually fit the data just as well.
\item Attaching a measure of ``confidence'' to a ``best fit'' requires
  knowledge or assumptions about the processes that generate the
  differences between model and data.
\end{itemize}
As one attempts to address these issues, more subtle challenges arise
and one can quickly become entangled in statistical philosophy and
reach the limits of the state of the art.  Making meaningful
mechanistic inferences from ODE fits to data may be unrealistic.
...
painless.  Simultaneously, we aim to bring to front and centre the
