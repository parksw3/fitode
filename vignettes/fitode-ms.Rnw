...
\item[initial conditions]{if all variables are observed, we can
    ...
    $\hat \state(0) = \state(0)$. Otherwise we have to estimate ICs along with other model parameters \citep{bjornstadEpidemics2018}.\djde{definition of auxiliary parameters? reference?} \swp{Is this writing clearer?}}
\end{description}
...
In general, using the log-likelihood is mathematically more convenient and numerically more stable than using the plain likelihood because most of the terms can be expressed as sums, rather than products---for example, multiplying many small likelihood values is likely to lead to imprecise calculations and therefore numerical instability \djde{Why?} \swp{Is this good enough? Neither Bjornstad's or Ben's book explicitly say that this is the reason so I'm not sure if we can cite them?}.
Using the negative log-likelihood provides further advantanges: (1) most optimization tools are designed to minimize, rather than maximize, objective functions and (2) the negative log-likelihood function is required for quantifying uncertainties in parameter estimates \djde{such as?  why would such a transformation change
the statistical properties?}. \swp{Is this good enough?}
