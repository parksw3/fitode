...
\end{equation}
where the parameter set $\params$ now includes parameters of the ODE model, initial conditions, and the variance term $\sigma^2$ of the Gaussian distribution as an auxilliary parameter (i.e., $\sigma^2 \in \params$).
