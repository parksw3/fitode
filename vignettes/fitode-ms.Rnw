...
For example, we can consider the sum of squared differences between the two.
...
Suppose we have (1) a set of gradient functions $\frac{d\state}{dt}$; (2) a method for numerical integration; (3) a set of observations $\state(t_i)$ where $i = 1, \dots, n$ (for simplicity, from a single time series with $n$ observations); (4) a method for numerical minimization of an objective function.
Define a trajectory (a solution of the ODE for specified parameters $\params$ [and initial conditions (ICs)] and time points $t_i$) as $\traj(t_i|\params)$. \bmb{notation???}\swp{changed some notations}
Then, the least-squared fitting seeks to find a set of parametrs, $\hat \params$, that minimize the following objective function: $\sum_{i=1}^n \left( \state(t_i) - \traj(t_i|\params)) \right)^2$.
...
Least-squares fits provide solutions that are in some sense ``optimal'' (although the Gauss-Markov theorem only applies to \emph{linear} models), but don't provide any scope for inference.
That said, by avoiding inference, one also avoids having to think much about the characteristics of the observations (independence, distribution, \ldots).
...
@
\noindent
\cite{murphy2016differences} analyzed this data set by fitting seven different ODE models (i.e., exponential, Mendelsohn, logistic, linear, surface, Bertalanffy, and Gompertz) via least-squares and comparing their model predictions.
Here, we consider the exponential growth model:
\begin{equation}
\frac{dA}{dt} = rA,
\end{equation}
...
%% https://bmccancer.biomedcentral.com/articles/10.1186/s12885-016-2164-x#Sec2
