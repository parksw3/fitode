...
\end{equation}
where the parameter set $\params$ includes the variance term $\sigma^2$ of the Gaussian distribution as an auxilliary parameter (i.e., $\sigma^2 \in \params$).
...
Equivalently, the MLE can be expressed as a parameter that minimizes the negative log-likelihood function (using the negative log-likelihood results in a more stable numerical computation and useful statistical properties):
\begin{equation}
