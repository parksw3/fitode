...
(Table~\ref{tab:bombay.fitode}) can be obtained via \code{coef} and \code{confint} functions;
...
time intervals using equation~\ref{eq:correct.mortality2}.
...
least squares (Figure~\ref{fig:phila}).  However, the estimated
parameters are extreme, with an $\mathcal R_0$ of 2310
(Table~\ref{tab:philanls}).  \swp{Again, possibly something wrong with
...
estimates (Table~\ref{tab:phila.fitode}).  Converting the SIR
...
$\R_0=\Sexpr{R0}$,
infectious period ($\gamma^{-1}$) $=\Sexpr{1/mm$gamma}$ week.  Figure~\ref{fig:stoch} (top
...
The approximation predicts a much lower epidemic peak and a shorter duration of the epidemic (Figure~\ref{fig:stoch}, top panel, blue curve).  \swp{Parameter values are a little screwed up at the moment. Doesn't seem like it's a trivial fix.}  Nonetheless, we can still fit the KM approximation to the simulated epidemic and obtain visually reasonable agreement (Figure~\ref{fig:stoch}, bottom panel, red curve; Table~\ref{tab:stoch.nls}); the fit cannot get better because the approximation \eqref{eq:sech} is symmetric about the peak, whereas the rise is steeper than the fall in the true solution.  \swp{we don't want to interpret these parameter estimates as being ``wrong'' until we figure out the parameter conversion issues.}

The exact deterministic SIR model gives a much better fit, which
is achieved by estimating an initial prevalence that is only a third of the true initial prevalence; all other parameter estimates are nearly identical to the true parameter values used to generate the stochastic trajectory (Table~\ref{tab:stoch.fitode}).
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      Figure~\ref{fig:stoch}).  The true parameter values used for
      the simulation were  $(S_0,I_0,R_0)=(\Sexpr{iii})$,
