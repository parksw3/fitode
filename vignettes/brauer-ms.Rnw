...
To estimate CIs, it will help to think about the probability of
observing the data $\{y_i\}$.  We imagine that the model
...
and the observed data).  Then the probablity of the data given the
model is
%%
\begin{linenomath*}
...
\end{linenomath*}
%%
But this isn't what we actually want. What we're given is the data,
not the model.  We're trying to say something about how likely our
\emph{proposed} model is, given the data that \emph{were} observed.
The typical way forward is to shift to a heuristic argument: it is
reasonable to consider parameter values that maximize the probability
of observing the data to be ``best'' or ``most likely'', so let's
simply \emph{define} the \term{likelihood} $\lik$ of a set of
parameter values $\thetavec$ to \emph{be} the probability of the data
given $\thetavec$,\bmb{I think we can say ``this is how it's typically
  done'' and give a reference without getting too far into the
  weeds.}\djde{Can you add whatever ref(s) you think are best here?}
\begin{linenomath*}
...
assumption of normal observation errors in the time series.
...
uncertainty in $\thetavechat$ and obtain confidence bands on the
fitted trajectory?

Our data are actually non-negative, discrete counts of deaths (or
cases in other contexts), so the observation error distribution can't
really follow a continuous, real-valued Normal distribution (in our
previous example, the assumption of normality is allowing the lower
limit of the confidence band on the fitted model to drop below zero
when the number of deaths is small).  We could assume a Poisson error
distribution with mean equal to the fitted model trajectory
[equation~\eqref{eq:SIR;R}], but typical observations have variance
greater than the mean (the variance is equal to the mean for a Poisson
distribution).  The negative binomial distribution can be considered
an overdispersed Poisson, which makes it a good choice.

The probability mass function for the negative binomial distribution
is
\begin{linenomath*}
\begin{equation}
\texttt{NB}(k;r,p) = \frac{\Gamma(k+r)}{k!\Gamma(r)}
(1-p)^k p^r \,,\qquad k=0,1,2,\ldots,
\end{equation}
\end{linenomath*}
where the parameters $\thetavec=(r,p)$ are related to the mean $\mu$
and variance $\sigma^2$ via
\begin{linenomath*}
\begin{equation}
...
\begin{linenomath*}
\begin{subequations}\label{eq:negbinomMLE}
\begin{align}
\thetavechat
&= \argmin_\thetavec \sum_{i=1}^n \Big(\!\!
-y_i \log\big(1-p_i\big) - 
               r_i
               \log{p_i}
               \Big)
 \,, \\
p_i &= \frac{\sigma^2-f(t_i;\thetavec)}{\sigma^2} \\
r_i &= \frac{f(t_i;\thetavec)^2}{\sigma^2-f(t_i;\thetavec)}
\end{align}
\end{subequations}
\end{linenomath*}
where, as for the likelihood associated with Normal errors, we assume
$\sigma$ is computed as the variance of the residuals across the full
...
to use the the curvature of $\log{\lik(\thetavec)}$ at
$\thetavechat$ to infer parameter values of a multivariate normal
distribution for $\thetavec$.  At $\thetavechat$, the shape of
$\log{\lik}$ is described by its \term{Hessian matrix} (the matrix
of second order partial derivatives of $\log{\lik}$, also known as
the \term{Fisher information matrix}), and the inverse of the Hessian
is the \term{variance-covariance matrix} $\mathrm{Cov}(\thetavec)$ that
specifies the desired multivariate normal with mean $\thetavechat$.
\djde{What is a simple way to explain why the inverse of the Hessian
  is the cov matrix for the parameters?}
\bmb{I don't think we can do this simply - we could point to
  \citep{Bolk08} for a heuristic explanation or \cite{Wassermanall2010}
  for a rigorous (if terse) explanation}

The diagonal elements of $\mathrm{Cov}(\thetavec)$ are the (estimated)
variances of the parameter estimates, so we can take
their (positive) square roots to get the standard error (SE) and
compute approximate 95\% confidence intervals by adding $\pm1.96\,\mathrm{SE}$ to $g(\thetavechat)$.
To obtain CIs
on functions of the fitted parameters (e.g., $\R_0$ or $\gamma$ if our
model is KM's approximation \eqref{eq:sech}), we build on the idea
that if the error in a parameter $a$ is $\Delta a$, then the associated
error in a (differentiable) function $g(a)$ is
$\Delta{g}\approx g'(a)\Delta{a}$.  Given a nonlinear function $g(\thetavec)$
of the parameters, the \term{Delta Method}
\cite{Dorf38,VerH12} expands $\textrm{Var}(g(\thetavec))$ to first
order about $\thetavechat$, which gives us the variance-covariance matrix
of $g(\thetavec)$ \cite{Bolk08}.  In particular, the variance of
$g(\thetavec)$ is
\begin{linenomath*}
...
\end{subequations}
\end{linenomath*}
We can again get the confidence intervals by taking square roots and computing
...
\cite{raue2013lessons}\footnote{Sensitivity equations are an auxiliary set of ODEs that define the time derivatives of the derivatives of the state variables with respect to the parameters (i.e. $\frac{d}{dt}\left(\frac{dx(t,\theta)}{d\theta}\right)$). They can easily be derived using the chain rule; by solving them in parallel with the main set of ODEs, we get a computationally efficient and numerically stable way to calculate the overall gradients of the log-likelihood with respect to the parameters. \bmb{this explanation can go in the main text if that seems more appropriate}}
...
phila1918b <- transform(phila1918a,
    date=c(as.Date("1918-09-09"), date),
    mort=c(NA, mort)
)
...
of $\thetavec$, under the assumption of normal observation errors in
the time series.
\bmb{I feel like we can introduce the idea of likelihood and maximum
  likelihood without presenting Bayes' rule. The justification for likelihood-based
  inference is most often presented heuristically rather than formally
  (i.e. we don't say ``here's Bayes' rule, we don't want to go full Bayesian,
  but here's something else we can do'' --- instead ``it is reasonable
  to suppose that parameter choices that maximize the probability of observing
  the data are `good' in some sense''). More generally, I think we can say
  ``this is how it's typically done'' and give a reference without getting too
  far into the weeds.}
