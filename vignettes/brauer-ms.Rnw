...
sech_approx <- function(t, a, omega, phi) {
    a * sech(omega*t - phi)^2
}
f <- function(t) sech_approx(t, a=890, omega=0.2, phi=3.4)
tvals <- seq(0,33,length=100)
plot(tvals, f(tvals), bty="L", col="darkred", type="l", lwd=3, xpd=NA,
     ylim=c(0,900),
     xaxs="i", yaxs="i", las=1, xlab="weeks", ylab="plague deaths")
points(mort ~ week, data = fitode::bombay, xpd=NA)
@ 
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July 1906.
  \emph{cf.}\ chart in \cite[p.\,714]{KermMcKe27}.}
\label{fig:Bombay}
\end{figure}
...
This curve does appear to provide a good fit to the data, but Kermack
and McKendrick \cite{KermMcKe27} gave no indication of how their
parameter estimates were obtained.  \djde{When was least-squares
...
\sum_{i=1}^n (f(t_i;\boldbeta) - y_i)^2 \,,
\end{equation}
\end{linenomath*}
where the observed data are the points $\{(t_i,y_i)\}$,
...
aid of a computer, as in the following R code.

\djde{insert code chunk that does it}

... explain normal CIs from least squares and how least squares is max
...
\djde{We must comment on whether the estimated $\beta$ and $\gamma$
  ...
  But also not KM's caution about interpreting the param values.}
...
\begin{table}
  ...
  \begin{tabular}{l | c | c | c | c}
    \bfseries Estimated parameter & \bfseries symbol & \bfseries units & \bfseries KM & \bfseries us \\\hline
    peak removal rate & $a$ & 1/weeks & 890  & \\
    outbreak speed & $\omega$ & 1/weeks & 0.2 & \\
    outbreak centre & $\phi$ & -- & 3.4 & \\
    \noalign{\vspace{10pt}}
    \bfseries Derived parameter \\\hline
    peak time & $\tpeak = {\phi}/{\omega}$ & weeks & 17 \\
