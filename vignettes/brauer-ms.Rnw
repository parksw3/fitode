...
\begin{abstract}
  ...
  has limited experience with statistical methodology or
...
\bmb{is it really necessary \ldots Can't we leave it as
...
\bmb{I took out the ref to \cite{Earn+20}; not sure why it was
  needed here? (We do cite it elsewhere, when discussing epidemic
  fitting windows}
...
reproduction number $\R_0\approx \Sexpr{signif(R0.phila.nls,2)}$ and a mean generation interval
$\Tg\approx \Sexpr{signif(Tg.phila.nls,2)}$ years.
...
\caption{\textbf{Unidentifiability of the mean generation interval
  ...
  $\gamma$. Panel \textsf{C} shows that the primary driver of
  unidentifiability is the confounding between $\gamma$ and $S(0)$.}\label{fig:proflikgamma}
\end{figure}
