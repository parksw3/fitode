...
We also analyze the influenza outbreak in Philadelphia, 1918.
Using nonlinear least squares, we can find a set of parameters
for KM's approximation that matches the epidemic curve
least squares (\cref{fig:phila}).  However, the estimated
parameters are extreme, with an $\mathcal R_0$ of 2490
(\cref{tab:philanls}).  The SIR model gives a visually similar fit as
the KM's approximation, while providing much more realistic parameter
estimates (\cref{tab:phila.fitode}).  Converting the SIR
parameters to KM parameters gives a worse fit, which underestimates
the size and timing of the peak.
...
\section{Example for which \KM approximation is poor
  (via stochastic  SIR)}
  ...
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      \cite[p.\,408]{bacaermodel2012}.\djde{Why are the units of
        ...
        in the previously line and -- and 1.7 got swapped ???}}\label{tab:bombay.fitode}
    \medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
...
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July
  ...
  panel\djde{No, in the top panel the yellow dotted curve is a smooth,
  ...
  \begin{center}
    \caption{Parameter estimates for the 1918 influenza epidemic in
      ...
      \Sexpr{prettyNum(philapop1918, big.mark=",")}.  \djde{I filled
        ...
        anything.  Should we make more of this explicit?}  See
      \cref{sec:phila,fig:phila}.}\label{tab:philanls}
    \medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
  \begin{tabular}{ m{3cm} | c | c | c | c c}
...
\caption{The main wave of the 1918 influenza epidemic in the city of
  ...
  See \cref{tab:philanls,tab:phila.fitode}.
  The blue and red curves show the \KM approximation
  \eqref{eq:sech}, based on the true parameters (blue) and
  fitted by us using \code{nls}
  (red, with pink confidence band).  The associated parameter
  estimates are given in \cref{tab:philanls}.
  The dotted yellow
  curve shows the \code{fitode} fit, for which the associated
  parameter estimates are given \cref{tab:phila.fitode}.
}
\label{fig:phila}
\end{figure}
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      the simulation were  $(S_0,I_0,R_0)=(\Sexpr{iii})$,
$\R_0=\Sexpr{R0}$, $1/\gamma=\Sexpr{1/mm$gamma}$ week.}\label{tab:stoch.fitode}
\medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
  \begin{tabular}{ m{3cm} | c | c | c c}
