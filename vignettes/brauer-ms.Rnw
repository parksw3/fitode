...
\sum_{\tindex=1}^{\tindexmax} \big(\traj(\ti;\thetavec) - y_\tindex\big)^2 \,,
\end{equation}
\end{linenomath*}
where the observed data are the points $\{(\ti,y_\tindex):\tindex=1,\ldots,\tindexmax\}$,
...
\;+\; \text{constant}
\Big) \\
&= \argmin_\thetavec\sum_{\tindex=1}^{\tindexmax} \big(\traj(\ti;\thetavec) - y_\tindex\big)^2
 \,,
\end{align}
\end{subequations}
\end{linenomath*}
