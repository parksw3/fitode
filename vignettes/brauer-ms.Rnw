...
years, as the power of computers has grown exponentially, there has
...
[\emph{cf.}\ \eqref{eq:leastsquares}\eqref{eq:sechcorrect}].  The
...
parameters to maximize the likelihood \citep{Bolk08}.  By calculating
...
estimated using the likelihood ratio test.\djde{we should either spell
  ...
  way} While profile likelihoods generally give more accurate
...
parameters or epidemic trajectories.  Consequently, we rely on the
...
Note that \code{fitode} gives discrete prediction (rather than smooth curves) because we are calculating mortality at discrete (weekly)
...
\djde{I've uncommented this comment, which occurs between the
  \texttt{\upshape sirmortfitcoefols} and
  \texttt{\upshape\detokenize{sirmortfit_ols_confband}} chunks, and I don't
  know which you were referring to.  If a comment is not longer
  relevant, then remove it.  If it is still relevant, then don't
  comment it out.}
...
\swp{I wonder if we should show the Philadelphia fits first.}
\djde{That would require some significant restructuring, and change
  the narrative.  I get why that would be advantageous, but I am
  not enthusiastic about changing the story so much at this point.}
  \swp{Sorry, I meant Philadelphia fit before the stochastic simulation.
  But not before Bombay. So Bombay first, then Philly, and stochastic.
  I don't think that would require much change in the story?}
...
\swp{need a proper time unit?}  We now apply the same methodologies to
a simulated mortality curve.  In particular, we simulated a stochastic
...
Finally, we analyze the influenza outbreak in Philadelphia, 1918.  At
this point, it is not surprising that we can find a set of parameters
for KM's approximation that matches the epidemic curve using nonlinear
