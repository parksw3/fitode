...
\begin{linenomath*}
  \begin{equation}\label{eq:prob.data.given.model}
    \begin{split}
      ...
      & = \lim_{\Delta x \to 0} \frac{\Pop\big(\traj(\ti;\thetavec) \le \trajobsti < \traj(\ti;\thetavec) + \Delta x \big)}{\Delta x} \\
      & = \prod_{\tindex=1}^n
