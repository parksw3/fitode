...
\name{David J.\,D.~Earn\textsuperscript{a}\thanks{CONTACT D.J.D.~Earn. Email: earn@math.mcmaster.ca. ORCID:
    ...
    Sang Woo Park\textsuperscript{b}\thanks{S.W.~Park. ORCID 0000-0003-2202-3361, Mastodon: @sangwoopark@ecoevo.social},
...
\bmb{is it really necessary \ldots Can't we leave it as
  ...
  \swp{I also agree that this doesn't feel necessary...}
%%
...
in purple.  The \code{fitode} fit (based on $\int (\dbydt{R}) \dt$) and
...
include \code{pomp} \citep{king2015statistical} and \code{mcstate}
\citep{mcstate}.
...
\begin{table}
  \begin{center}
    \caption{Parameters of the SIR model \eqref{eq:SIR}, as estimated
      ...
      (\code{ols}).  \djde{Why are there no CIs for \code{ols}?} \swp{Numerical
      instability caused by unidentifiability leading to not being able to get the
      right variance covariance matrix..} The
      ...
      parameter unidentifiability.  \djde{Can we say what goes wrong
        ...
        \code{fixed} argument in the main text.} $\gamma$ was fixed to
        $4.11/\textrm{weeks}$ to remain consistent with \code{nls} fits.
      %%
