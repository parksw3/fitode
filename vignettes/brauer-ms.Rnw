...
\swp{Think we need to add some discussion of profile likelihood here:}
\djde{thx. revised a bit} Better confidence intervals can be obtained
...
In this case, \KM's approximation \eqref{eq:sech}---using the correct parameter values---yields a poor representation of the dynamics.\djde{The fitted
  $\R_0=0.0892$, which is absurd.  It should not be giving $\R_0<1$.}  The approximation predicts a much lower epidemic peak and a shorter duration of the epidemic (Figure~\ref{fig:stoch}, top panel, blue curve).  \swp{Parameter values are a little screwed up at the moment. Doesn't seem like it's a trivial fix.}  Nonetheless, we can still fit the KM approximation to the simulated epidemic and obtain visually reasonable agreement (Figure~\ref{fig:stoch}, bottom panel, red curve; Table~\ref{tab:stoch.nls}); the fit cannot get better because the approximation \eqref{eq:sech} is symmetric about the peak, whereas the rise is steeper than the fall in the true solution.  \swp{we don't want to interpret these parameter estimates as being ``wrong'' until we figure out the parameter conversion issues.}
