...
col.fitode.ols <- "orange" # my.orange
...
\begin{table}
  \begin{center}
    \caption{Fits of \KM's \cite{KermMcKe27} analytical SIR
...
\begin{table}
  \begin{center}
    \caption{Fits of numerical SIR model solutions to Bombay plague
      (see \cref{fig:Bombay}).  Parameters values were estimated using
      \code{fitode} to fit trajectories of \cref{eq:SIR}, assuming
      observation errors were distributed negative binomially
      (\code{dnbinom}) or normally (\code{ols}).  The recovery rate
      ...
      comparison of the fits.  We do not report CIs associated with
      our \code{ols} fit in the final column because fixing $\gamma$
      was not sufficient to yield numerically stable calculations of
      CIs in this case.  \swp{this is quite random; if I start from
        some other starting value, ols works fine but dnbinom doesn’t
        work} \djde{Are the estimates and CIs you get with the
        different starting conditions reasonable? Is the lesson that
        with weak data it may be necessary to fiddle with starting
        values to obtain decent fits including CIs?}
      %%
      The \code{nbinom}-fitted trajectory and confidence band, and
      the \code{ols}-fitted trajectory, are shown in the lower panel
      of \cref{fig:Bombay}.
      %%
...
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July
  ...
  yellow curve shows the \code{fitode} fit of the SIR model
  \eqref{eq:SIR}, for which the associated parameter estimates are
  given in \cref{tab:bombay.fitode} (observation errors are assumed to
  be negative binomially distributed; see \cref{sec:Uncertainty}).
  %%
  \emph{Bottom panel:} The solid yellow curve is identical to the
  dotted yellow curve in the top panel, and the light yellow band
  shows the \code{fitode} confidence band; this confidence band is
  shown as a linear interpolation between successive observation times
  because the model \eqref{eq:SIR} is fitted to incidence at discrete
  ...
  solid red curve in the top panel. The orange curve shows the
  \code{fitode} fit obtained by minimizing the ordinary least squares.
  We have separated the two panels because the confidence band overlap
  would make the plots difficult to interpret.}
\label{fig:Bombay}
\end{figure}
...
\begin{table}
  \begin{center}
    \caption{Fits of numerical SIR model solutions to Philadelphia flu
      (see \cref{fig:phila}).  
...
\begin{table}
  \begin{center}
    \caption{Fits of \KM's \cite{KermMcKe27} analytical SIR
      ...
      %%
      The parameter values in the ``True'' column are
      are 
      those used to
      generate the stochastic simulation ($S_0$, $I_0$, $\R_0$
      and $\Tg$) and the values of other parameters derived
      from these true parameter values using the indicated equations.
      \djde{A value for every single parameter should be listed in this
        column with no exceptions.}
      %%
      The \code{nls} column lists our estimates obtained by fitting
      \cref{eq:sech} to the simulated data using nonlinear least
      squares and the Delta method.
      %%
      \djde{Daniel, I am still failing to understand what the \KM
        column is.  We need to talk about this.}
      %%
}\label{tab:stoch.nls}
    \medskip
    %% making use of ragged2e and array packages:
...
\begin{table}
  \begin{center}
    \caption{Fits of numerical (deterministic) SIR model solutions to
      ...
      the determinsitic curve were generated by negative binomially
      distributed observation errors.  }\label{tab:stoch.fitode}
\medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
  \begin{tabular}{ m{3cm} | c | c | c | c c}
