...
\code{fitode} can
\begin{description}
\item [fit to multiple data streams]: e.g. in the case where
  case reports, hospitalization reports, and wastewater sampling are
  all available in a COVID-19 time series
...
\item [apply Bayesian inference]:
  ...
  but are usually mush more computationally expensive.
  ...
  \emph{Markov chain Monte Carlo} algorithms are typically used to explore the
  parameter space and approximate the target distribution. \code{fitode} implements
