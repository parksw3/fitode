...
\begin{table}
  ...
  \begin{tabular}{ m{3cm} | c | c | c c | c c}
    ...
    transmission rate & $\beta$ & $\frac{1}{\textrm{weeks}}$ &
      \Sexpr{beta.bombay.fitode.nb} & \Sexpr{ci_fmt("beta", ci.bombay.fitode.nb)}  & ?? & ?? \\
    initial\break susceptibles & $S_0$& -- &
      \Sexpr{as.integer(S0.bombay.fitode.nb)} & \Sexpr{ci_fmt("S0", ci.bombay.fitode.nb, max_ord = 6)} & ?? & ?? \\
      initial prevalence & $I_0$ & -- &
      \Sexpr{I0.bombay.fitode.nb} & \Sexpr{ci_fmt("I0", ci.bombay.fitode.nb)} & ?? & ?? \\
