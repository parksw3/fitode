...
$\log{\lik}$ is determined by its \term{Hessian matrix} (also known as
the \term{Fisher information matrix}), and the inverse of this matrix
...
that if the error in a variable $x$ is $\Delta x$, then the associated
error in a (differentiable) function $f(x)$ is
$\Delta{f}\approx f'(x)\Delta{x}$.  The \term{Delta Method}
\cite{Dorf38,VerH12}
allows us to approximate the variance-covariance matrix
of any non-linear transformation of parameters $g(\betavec)$ \cite{Bolk08}:
\begin{linenomath*}
\begin{equation}
\mathrm{Var}(g(\betavec)) \approx \nabla g(\betavec)^{\textsf{T}} \cdot \mathrm{Cov}(\betavec) \cdot \nabla g(\betavec)
\end{equation}
\end{linenomath*}
\djde{works not only for derived parameters such as $\R_0$, but also
  for the trajectory itself... because $f(t_i,\betavec)$ is just a
  function of the parameters...  hence confidence band on the
  trajectory}
