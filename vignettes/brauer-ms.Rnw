...
\djde{We should give $\beta$ in units of 1/years instead of 1/weeks.
   Then $\beta\simeq0.00425/$yr, which doesn't require scientific notation.}
\swp{ci_fmt keeps giving me scientific notation for CIs. Not sure if we want to change that manually or keep it as is.}
}\label{tab:Bombay}
    \medskip
  ...
  \bfseries Assumed\break parameter & {\footnotesize\bfseries symbol}
    ...
    transmission rate & $\beta$ & \eqref{eq:beta} & $\frac{1}{\textrm{years}}$ &
      \Sexpr{signif(beta.KM*52,dig)} & \Sexpr{signif(beta.nls*52,dig)} & \Sexpr{ci_fmt("beta", cc.delta*52)} \\
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      \djde{Again, units of $\beta$ should be 1/year.}
  ...
  \begin{center}
    \caption{Parameter estimates for the 1918 influenza epidemic in
      ...
      \Sexpr{prettyNum(philapop1918, big.mark=",")}.  \djde{I filled
        ...
        anything.  Should we make more of this explicit?}  \djde{Why
        did we \underline{assume} $I_0=\Sexpr{I0.phila.fitode}$ ??}
      See \cref{sec:phila,fig:phila}.
      \djde{Units of $\tpeak$ should be weeks or days, not years.}
      \djde{The CI for $\R_0$ should not be in scientific notation.
      4 digit integers are far better.}
    }\label{tab:philanls}
    \medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
...
\begin{table}
  \begin{center}
    \caption{Parameters of \KM's \cite{KermMcKe27} approximation
      \eqref{eq:sech} to the SIR model \eqref{eq:SIR}, as estimated by
      us using original parameters that were used to simulate
      the stochastic SIR model
      \djde{I don't understand that.  What does the KM column
      mean exactly?}
      and using nonlinear least squares
      (\code{nls}).
      \djde{Rather than the following sentence, I think it would be better
        to have a ``true'' column in the table and fill in all the
        rows for the true parameter values.}
      The true parameter values used for
