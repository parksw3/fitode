...
\begin{abstract}
  Fred Brauer made many contributions to mathematical epidemiology,
  but had no desire to touch data.  Nevertheless, he recognized that
  ...
  package, \code{fitode}, that facilitates fitting ordinary differential equations to
  observed time series.  Here, we use this package to
  ...
  \bmb{Is this framing weird if this is going to appear in a regular issue of BMB? Does it need
    to be tweaked, e.g. with a clause explaining who Fred Brauer was?}
\end{abstract}
...
susceptible, infected or removed\footnote{In the words of \KM
  ...
  are sick, by recovery or by death''. \bmb{is this needed?}}, $\beta$ is the transmission
...
very sensitive to the starting value.  To pick reasonable starting values,
it often helps to think
about the meaning of parameters.  For example, in the case of
\cref{eq:sech}, it is useful to note that $a$ is the maximum
...
$\tpeak$ can be approximated by looking at the plotted data.
...
Very rough guesses for $a$, $\tpeak$ and $\omega$
...
you pick sufficiently \emph{bad} starting values, then \code{nls} will fail.
For example, starting from $a=2000$, $\tpeak=5$, and $\omega=0.1$
yields a ``singular gradient'' error.  More interestingly, starting
...
nonlinear optimizers to converge to points that may or may not
represent equally good fits to the data.  The potential existence of
multiple local optima makes fitting models to data hard; you need to
be cautious, and use common sense, in interpreting the solutions found
by your software (always plot the solutions!). Raue \emph{et al.}\/
\cite{raue2013lessons} give suggestions for how to diagnose and handle
multiple optima.
...
trick, there is more to it than that.  A much broader range of
...
or inverse of the logistic function), which converts the unit interval $(0,1)$ to
...
\cref{eq:sechparams} can be inverted\footnote{Our expressions are
  ...
  this missing square propagated through to the inversion formulae.}
...
\cref{eq:invertparams}, we can compute the transmission
rate,
\begin{linenomath*}
...
\bmb{is it worth mentioning the assumption that infection fatality
  rate $\approx$ 100\% again (``Note that we are still assuming
  \ldots''?)}
\djde{I've added two sentences above.  Feel free to improve, and to
  comment on the non-indentifiability problems or whatever you think
  is useful to mention here.}
\bmb{Maybe add further that we're ignoring a lot more biology.
\emph{cf.} Steve Ellner's fit of the Bombay plague data where
he said he was assuming a constant proportionality between
rat prevalence and human death.}
\djde{Can you point me to the Ellner paper?}
\swp{I couldn't find the paper... I'll leave this to Ben.}
...
model trajectory [\cref{eq:SIR;R}]. This handles
...
\end{equation}
\end{linenomath*}
The predicted variance of a particular observation $y_i$ is given by
$\mu_i (1+\mu_i/k)$, where $\mu_i(\thetavec) = f(t_i;\thetavec)$ is the
model evaluated at the $i$\textsuperscript{th} observed data point
[\emph{cf.}\ \eqref{eq:leastsquares} and \eqref{eq:sechcorrect}].
...
standard Normal distribution).  To obtain CIs on functions of the
...
alongside the main differential equations. Sensitivity equations are
an auxiliary set of ODEs that define the time derivatives of the
...
the components of $\thetavec$. They can easily be derived using the
chain rule; by solving them in parallel with the main set of ODEs, we
get a computationally efficient and numerically stable way to
calculate the overall gradients of the log-likelihood with respect to
the parameters.
\djde{You define the derivatives but not the \underline{equations}.
  It isn't clear to me from above what he sensitivity equations are
  and how solving them helps, and certainly not how it yields
  efficiency and numerical stability.}
\swp{Not sure how much more detail is needed. But sensitivity equations correspond to the following (is this what you meant by equations?):
\begin{equation}
\frac{\rm d}{{\rm d}t}\left(\frac{\partial
  x(t,\theta_i)}{\partial\theta_i}\right) = \left[\frac{\partial}{\partial x} \left(\frac{{\rm d} x(t,\theta_i)}{{\rm d} t} \right)\right] \frac{\partial
  x(t,\theta_i)}{\partial\theta_i} + \frac{\partial}{\partial \theta_i} \left(\frac{{\rm d} x(t,\theta_i)}{{\rm d} t} \right)
\end{equation}.
There are two reasons why solving sensitivity equation helps. First, it allows us to use gradient-based optimization algorithms.
Second, it provides us the gradient of the epidemic trajectory with respect to model parameters, which can be then used to calculate CIs using the Delta method.
It would be possible to do all of this using numerical differentiation (i.e., by taking the difference between $x(t,\theta_i)$ and $x(t,\theta_i+\epsilon)$) but this would be less stable.
Comparison between sensitivity equation vs numerical differentiation is shown in \cite{raue2013lessons}.
}
...
$R(t_i)$).  Ordinary least squares can be implemented by changing the \code{observation} argument to
{\tt
  \hlstd{mort}
  \!\!{\color{black}\~{}}\!\!
...
\end{description}
Since we are taking the difference $\mu(t_i) = R(t_i) - R(t_{i-1})$ to
calculate the mortality trajectory,\footnote{It is not unusual to see
  fits to cumulative curves.  However, doing so is ill-advised because
...
@
%%
\bmb{is it worth a note/footnote here to caution about the common
  practice of fitting to cumulative curves, e.g.
  \cite{King+15}?}\djde{I've added a footnote above; you may want to
  alter it.}  Taking our previous parameter estimate from \code{nls}
as our starting values, we can fit the model by calling the
\code{fitode} function:
<<sirmortfit,cache=TRUE,warning=FALSE,message=FALSE>>=
...
\item[\code{fixed}]specifies parameter values to be fixed (and
  therefore not estimated). Here, we chose to assume that the recovery
  rate is known (using the \code{fixed} argument) due to parameter
  unidentifiability\footnote{In short, unidentifiability of $\gamma$
    ...
    of $\gamma$. We discuss this issue in detail in Supplementary
    Materials.\djde{As I say elsewhere, I don't think we need supp.
      I think we should be able to explain whatever is needed in the
      caption to \cref{fig:proflikgamma}, or in an appendix if
      necessary.} \swp{I agree that figure caption is good enough.}}.
...
\term{coefficients} in standard statistical jargon) can be obtained
...
also allows users to obtain confidence intervals for derived
...
\djde{Is it really bad or is it just that we are assuming CFP$=1$?}
\swp{Really bad, I think. Look at the profile likelihood in the email. I think it's numerically bad, not necessarily biologically bad.}
\djde{I've added text below to connect this section to the flu
  section, and hopefully reduce concerns about the plague data.  Let
  me know if you think more needs to be said here.}
\swp{This seems good enough to me.}
...
epidemiological model to data, it is important to emphasize that
modelling plague dynamics with the simple SIR model is, at best,
difficult to justify \cite{bacaermodel2012,Earn+20}.  Indeed, \KM
explicitly made a sequence of optomistic assumptions, which they
recognized were not ``strictly'' satisfied:
...
epidemic in the 1920's would have been the 1918 influenza pandemic,
...
reproduction number $\R_0\approx2500$ and a mean generation interval
$\Tg\approx1.5$ years.

Matching trajectories of the exact SIR model using \code{fitode} gives
a fit---the yellow curve in \cref{fig:phila}---that is visually
...
particular, $\R_0\approx6.4$ and $\Tg\approx4.27$ days.  \djde{Ummm,
  that $\Tg$ is still way too big.}
  \swp{I accidentally multiplied by 7 in the table (which happened from copying the code for Bombay).}
...
underestimates the magnitude of the epidemic (the turnover is much too
...
\eqref{eq:KMassumption} on which it is based becomes less and less
valid.
...
\section{Fitting the deterministic SIR model to stochastic simulations}
...
to data that have been generated with a known algorithm, so we know
the underlying values of the parameters we are trying to estimate.
...
estimate the parameters.  At least if we start from parameter guesses
close to the correct underlying values, \code{fitode} should converge
to those values.

The next level of test would be to take our numerically computed
...
A still higher level of test is to begin by simulating data that are
more realistic than can be generated by the idealized model that we
want to fit, and then seeing if we can nevertheless recover the
underlying parameter values that correspond to those of our idealized
...
in purple.  The \code{fitode} fit and
...
\djde{Daniel, you wrote ``shows the simulated daily mortality curve
  (dots) alongside the smooth, deterministic trajectory of epidemic
  mortality ($\dbydt{R}$, green)''.  Am I correct that you did not
  model mortality?  So to interpret this as mortality, we would be
  assuming that the CFP$=1$?  If that's the case, I don't think we
  should describe this as mortality, but we might want to comment on
  how to simulate mortality data.}
\swp{Several things going on here...yes, I'm just showing the smooth dR/dt, but
I can show the integral of dR/dt if we want to. We also already comment on
how to simulate mortality in equation 21? I also feel like assuming CFP=1 is OK
for the simulation example but also don't feel strongly about not saying mortality...}
...
We focussed here on three illustrative examples of epidemic time
...
1918 influenza pandemic\djde{feel free to suggest other examples!}).
\swp{Not sure if model selection is the right way to go... this is
the exact problem that the other plague paper had, if you remember.
We could also mention Ben's multimodel paper as a caveat of this approach?}
...
computational challenge that---in the absense of additional
...
impossible to estimate reliably from the reported weekly plague deaths
(see \cref{fig:proflikgamma}).

\djde{We should not have supp.  There is just the one figure, which we
  can just stick here (I've commented out the creation of supp so it
  is now \cref{fig:proflikgamma} rather than S1).  We can probably say
  as much as we need to in the figure caption, but we could,
  alternatively, have an \emph{appendix}, rather than supplementary
  material.}
  \swp{I agree we don't need appendix or supp}
\djde{I wonder if fitting to the full 15 year time series that
  Baca\"er shows would make $\Tg$ identifiable.}
  \swp{Maybe...but another paper.}
...
series: we fit models to a \emph{simulation} that we ran, so we knew
...
\djde{and a discrepancy in the initial conditions that makes perfect
sense; awkward to get this across nicely}
(\cref{tab:stoch.fitode,tab:stoch.nls}).
\swp{How about this: while we still find a minor discrepancy in the estimates of
initial conditions, this discrepancy is expected as this particular realization of
the stochastic outbreak simulation did not take off immediately. Underestimation of
the initial prevalence allows the deterministic model to offset the delayed epidemic
onset.}

The reason that \KM's approximation fails to estimate the correct
parameter values for the simulation is that the assumption on which
the approximation is based \eqref{eq:KMassumption} is strongly
...
or mechanistically.  It is worth noting, more generally, that a purely
...
that a model can help explain patterns in data.

While not essential for conveying the basics that we have emphasized
here, we would be remiss in not mentioning that \code{fitode} contains
a number of useful features that we have not discussed.  In particular,
there are options for:
\djde{Perhaps mention the help commands that users should use to learn
how to take advantage of each of these options?} \swp{I don't think any of these
have any help commands yet... I can try to come up with them later...}
\begin{description}
\item[\term{multiple data streams:}]\code{fitode} allows for
  simultaneous fitting of multiple state variables to multiple time
  series.
\item[\term{importance sampling:}]\code{fitode} provides an option to
  compute confidence intervals using the following approach\needref.
  Parameter sets are randomly drawn from a multivariate normal
  distribution whose mean and variance-covariance matrix correspond to
  the maximum likelihood estimates and the estimated
  variance-covariance matrix, respectively; then, each parameter set
  is weighted accordingly based on the the likelihood of their
  epidemic trajectories and the sampling probability.  Generally, this
  method is expected to provide more robust confidence intervals than
  the Delta method\needref, but is computationally more costly.
  \swp{Ben might know references for these ones... I think we just sat down
  and derived them together without references?}
\item[\term{Bayesian inference\needref via MCMC\needref:}]
\djde{BB or SWP: Can you add something about why this
    option is useful.}
    \swp{Unlike maximum likelihood approaches, which seek to estimate the best-fitting
    ...
    assumptions/knowledge about the system (also known as the prior distribution) as well
    as the observed data. Markov chain Monte Carlo are typically used to explore the
    parameter space and approximate the target distribution. These approaches are generally
    better at handling parameter uncertainteis but are computationally more expensive in general.}
\end{description}
Even with these extensions, there will be many other challenges to
fitting ODE models with the \code{fitode} package, as with many other
software packages\djde{(e.g., ...????...)}.  For example, it is often
difficult to ensure that the model has converged properly or reached
its true maximum.  Performing optimization from multiple starting
conditions and drawing likelihood surfaces can help diagnose these
problems.  Using different optimization methods or reparameterizing
the model can sometimes help resolve these issues as well.  We also
encourage users of \code{fitode} who encounter these or other
fitting challenges to open any issues via the GitHub repository:
\url{https://github.com/parksw3/fitode}.

As its name suggests, the scope of \code{fitode} is limited to fitting
ODEs to time series.  Consequently, by design, \code{fitode} ignores
\term{process error}, i.e., variability that is intrinsic to the
structure of the system (as opposed to \term{observation error}, which
arises from imperfect measurements or reporting).  A key component of
process error is the demographic stochasticity that is inherent to the
stochastic SIR model discussed above (and to any real host-pathogen
system).  Parameters of models can also be subject to process error;
for example, the transmission rate might, in practice, have a
component that is dependent on random fluctuations in weather.  A
number of studies have found that accounting for process error can be
critical for estimating uncertainties in parameter estimates
and confidence bands on the projected dynamics of a system\needref \citep{King+15,taylor2016stochasticity,li2018fitting}.
Popular \code{R} packages that can fit models with process error
include \code{pomp} \citep{king2015statistical} and \code{odin}
\citep{odin2021}.
...
on its own \citep{raue2013lessons}, and estimating statistically meaningful
uncertainty in those optima is extremely challenging\citep{elderd2006uncertainty,li2018fitting}\needref.
...
\item Comment on reparameterizing, specifically parameterizing with $\tpeak$ or $\phi$ rather than initial condition $I_0$?
...
\item statistical challenges, where to go for further insights / more
  advanced approaches
\item fitting ABMs
\item switch to Bull Math Biol template
\end{itemize}

\bmb{pedagogical refs: I would use Raue et al \cite{raue2013lessons} for
...
($N\simeq10^6$).  \swp{The problem is that we're assuming S0 = 1e6 -
  ...
  from estimated parameter values.}
\bmb{Note that Baca\"er \cite[Table~2, p.\.412]{bacaermodel2012}
quotes CFP $\sigma = 0.9$ from the Indian report.  We can use
this to argue that an independent estimate of CFP seems to rule
out really small CFP.  In principle, we could do fit seasonal
forcing and CFP and argue that with a reasonable prior on CFP
we need seasonal forcing (or whatever).}
...
{\color{gray}
  ...
  \bfseries Assumed\break parameter & {\footnotesize\bfseries symbol}
    ...
    & {\footnotesize\bfseries KM} & {\footnotesize\bfseries \code{nls}} & {\footnotesize\bfseries 95\% CI} \\\hline
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      \cref{fig:Bombay}).  The $\R_0$ estimates assume the
      population of Bombay was $N=1$ million
      \cite[p.\,408]{bacaermodel2012}.
  ...
  \bfseries Assumed\break parameter & {\footnotesize\bfseries symbol}
    ...
    & {\footnotesize\bfseries \code{dnbinom}} & {\footnotesize\bfseries 95\% CI} & {\footnotesize\bfseries \code{ols}} \\\hline
    ...
    transmission rate & $\beta$ & $\frac{1}{\textrm{years}}$ &
      \Sexpr{beta.bombay.fitode.nb*52} & \Sexpr{ci_fmt("beta", ci.bombay.fitode.nb*52, sn=FALSE, digits=dig)}  & \Sexpr{beta.bombay.fitode.ols*52} \\
    initial\break susceptibles & $S_0$& -- &
      ...
      \bfseries Derived\break parameter \\\hline
      effective reproduction number & $\Reff$ & -- &
      \Sexpr{Reff.bombay.fitode.nb} &  \Sexpr{ci_fmt("Reff", ci.derived.bombay.fitode.nb[,-1])} & \Sexpr{signif(beta.bombay.fitode.ols*as.integer(S0.bombay.fitode.ols)/gamma.nls,dig)} \\
      mean generation interval & $\Tg$ & days &
      \Sexpr{signif(7*Tg.bombay.fitode.nb,dig)} & \Sexpr{ci_fmt("Tg", 7*ci.derived.bombay.fitode.nb[,-1])} & \Sexpr{signif(7/gamma.nls,dig)}  \\
    basic reproduction number & $\R_0$ & -- &
      \Sexpr{signif(R0.bombay.fitode.nb,dig)}
      &  \Sexpr{ci_fmt("R0", ci.derived.bombay.fitode.nb[,-1])} & \Sexpr{signif(beta.bombay.fitode.ols*1e6/gamma.nls, dig)}
  \end{tabular}
  \end{center}
\end{table}
\end{landscape}
...
points(mort ~ week, data = fitode::bombay, xpd=NA, pch=21, bg="white", lwd=2)
legend("topleft", bty="n", lwd=c(2,lwd,lwd),
       lty=c(NA,"dotted","solid"),
       col=c("black",col.nls,col.fitode.nb),
       pch=c(21,NA,NA),
       pt.bg=c("white",NA,NA),
       legend=c("observed data","nls","fitode (nbinom)"))
@
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July
  ...
  using \code{nls} (red, with pink confidence band).  The associated
  parameter estimates are given in \cref{tab:Bombay}.  The dotted
  yellow curve shows the \code{fitode} fit, for which the associated
  parameter estimates are given in \cref{tab:bombay.fitode}
  (observation errors are assumed to be negative binomially
  distributed; see \cref{sec:Uncertainty}).  \emph{Bottom panel:} The
  yellow curve is identical to the yellow dotted curve in the top
  panel\djde{No, in the top panel the yellow dotted curve is smooth,
    not interpolated from values at observation times.} \swp{I'm confused because they are identical? They're both using the same code too. See L2143 and L2157}, and the
  yellow band shows the \code{fitode} confidence band; this confidence
  band is shown as a linear interpolation between successive
  observation times because the model fits to incidence at discrete
  time points rather than to a continuous curve representation of the
  instantaneous death rate.  The dotted red curve is identical to the
  solid red curve in the top panel.  We have separated the two panels
  because the confidence band overlap would make the plots difficult
  to interpret.}
\label{fig:Bombay}
\end{figure}
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates for the 1918 influenza epidemic in
      Philadelphia, based on a nonlinear least squares (\code{nls})
      fit of \KM's \cite{KermMcKe27} approximation \eqref{eq:sech} to
      the SIR model \eqref{eq:SIR}.  The $\R_0$ estimates assume the
      population of Philadelphia in 1918 was
      \Sexpr{prettyNum(philapop1918, big.mark=",")}.  \djde{I filled
        in \code{philapop1918} there.  We also assumed
        $\code{philacfp}=\Sexpr{philacfp}$, which determined
        $\code{philaS0}=\Sexpr{philaS0}$.  But then we took the
        initial \code{S0} guess to be $0.9*\code{philaS0}$.  The
        fitted \code{S0} is so far from this that the assumed
        population size and CFP don't seem to be constraining
        anything.  Should we make more of this explicit?}
        \swp{Ah yes we should make this more explicit. Shouldn't we also fill out \code{philaS0} for R0 estimates instead o f \code{philapop1918}? As far as I can tell, we're using \code{philaS0} to get R0 estimates? For example, see L1359 and L1399.}
        \djde{Why
        did we \underline{assume} $I_0=\Sexpr{I0.phila.fitode}$ ??}
        \swp{It's actually what we estimate from fitode.  And we need to assume something about I0 to get other parameters.}
      See \cref{sec:phila,fig:phila}.
    }\label{tab:philanls}
    \medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
...
\begin{table}
  ...
  \bfseries Estimated\break parameter & {\footnotesize\bfseries symbol}
    & {\footnotesize\bfseries units}
    & {\footnotesize\bfseries \code{dnbinom}} & {\footnotesize\bfseries 95\% CI} \\\hline
    transmission rate & $\beta$ & $\frac{1}{\textrm{years}}$ &
...
legend("topleft", bty="n", lwd=c(2,lwd,lwd,lwd*0.60),
       lty=c(NA,"solid","dotted","solid"),
       col=c("black",col.nls,col.fitode.nb,my.orange),
       pch=c(21,NA,NA,NA),
       pt.bg=c("white",NA,NA,NA),
       legend=c("observed data","nls","fitode (nbinom)","KM approximation"))
@
\caption{The main wave of the 1918 influenza epidemic in the city of
  ...
  confidence band
  show a nonlinear least squares (nls) fit of \KM's
  approximation \eqref{eq:sech}; the parameter estimates are given in
  \cref{tab:philanls}.  The dotted yellow curve and yellow confidence
  band show the \code{fitode} fit, for which the parameter estimates
  are given in \cref{tab:phila.fitode}.  The blue curve shows the \KM
...
\begin{table}
  \begin{center}
    \caption{Parameters of \KM's \cite{KermMcKe27} approximation
      \eqref{eq:sech} to the SIR model \eqref{eq:SIR} calculated
      by directly converting the parameters that were used to simulate
      the stochastic SIR model and
    \djde{I don't understand that.  What does the KM column
      mean exactly?} \swp{We converted SIR model parameters to KM parameters. I tried to make this clearer.}
      and  by fitting nonlinear least squares
      (\code{nls}).
}\label{tab:stoch.nls}
    \medskip
    %% making use of ragged2e and array packages:
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      \cref{fig:stoch}).  The true parameter values used for
      the simulation were  $(S_0,I_0,R_0)=(\Sexpr{iii})$,
      $\R_0=\Sexpr{R0}$, $1/\gamma=\Sexpr{1/mm$gamma}$ week.
...
\medskip
  ...
  \bfseries Estimated\break parameter & {\footnotesize\bfseries symbol}
    ...
    recovery rate & $\gamma$ & $\frac{1}{\textrm{weeks}}$ &
    ...
    initial\break susceptibles & $S_0$& -- &
      ...
      \noalign{\vspace{10pt}}
      \bfseries Derived\break parameter \\\hline
      mean generation interval & $\Tg$ & days & \Sexpr{signif(7*1/gamma.stoch.fitode,dig)} & \Sexpr{ci_fmt("gamma", 7*1/ci.stoch.fitode)}\\
      effective reproduction number & $\Reff$ & -- &
...
legend("topright", bty="n", lwd=c(2,lwd,lwd),
       ...
       col=c("black","purple",my.green),
       pch=c(21,NA,NA),
       pt.bg=c("white",NA,NA),
       legend=c("observed data","KM conversion","deterministic"))
...
\emph{Top panel:} The purple curve shows the \KM approximation
  \eqref{eq:sech} calculated by directly converting the original
...
\caption{\textbf{Unidentifiability of the mean generation interval
  ...
  given value and maximizing the likelihood by estimating all other
  parameters \cite{Bolk08}.  A flat profile-likelihood surface
  indicates parameter unidentifiability, meaning that we can obtain
  statistically indistinguishable fits across a wide range of values
  of the focal parameter ($\gamma$).  (\textsf{B--E}) The
  corresponding best parameter estimates for a given value of
  $\gamma$.  Strong correlations across parameters are symptomatic of
  parameter unidentifiability.  \djde{The viz in the figure could be
    improved.  I suggest stretching panel A to cover the current
    positions of panels A and D, and then have a 4x4 grid of parameter
    estimate panels next to it.  Also, use math symbols in the annotation,
    e.g., $\beta$ not \texttt{beta}.}
}\label{fig:proflikgamma}
\end{figure}
