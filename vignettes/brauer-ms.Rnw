...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      \cite[p.\,408]{bacaermodel2012}.\djde{Why are the units of
        $\R_0$ ``1.7'' ???  I guess that is the ols value of $\Tg$
        in the previously line and I guess that
        ``--'' and ``1.7'' got swapped ???}
  ...
  \bfseries Assumed\break parameter & {\footnotesize\bfseries symbol}
    ...
    basic reproduction number & $\R_0$ & \Sexpr{signif(7*Tg.bombay.fitode.nb,dig)} &
...
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July
  ...
  yellow curve \djde{The legend should have dotted rather than solid
    yellow.} shows the \code{fitode} fit, for which the associated
  ...
  panel\djde{No, in the top panel the yellow dotted curve is smooth,
    not interpolated from values at observation times.}, and the
  ...
  to interpret.  \djde{In the legend, ``KM'' might be clearer as
    ``KM's (1927) fit'', and ``nls'' might be clearer as ``our nls
    fit''.}  }
\label{fig:Bombay}
\end{figure}
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits
      ...
      of Philadelphia.
      \djde{$\Tg$ should be given in units of weeks or days,
      not years.}
