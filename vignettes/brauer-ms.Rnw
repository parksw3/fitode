...
To understand how to estimate CIs, it will help to think about the
...
not the model.  \swp{this is a bit confusing to me. It's true that we're given the
data but the likelihood is exactly equal to the probability of data
given model (just a notational difference). Even
in Ben's book, he wrote: ``Our goodness-of-fit metrics will be
based on the likelihood, the probability of seeing the data we
actually collected given a particular model.'' So maybe better
to skip some of the details below?}
We're trying to say something about how likely our
...
given $\thetavec$,\bmb{I think we can say ``this is how it's typically
  ...
  \swp{Ben's book seems good enough to me.}
\begin{linenomath*}
...
assumption of \swp{independent} Normal observation errors in the time series.
...
cases in other \swp{epidemiological} contexts), so the observation error distribution can't
really follow a continuous, real-valued Normal distribution.
\swp{I find this s. slightly awkward because a lot of people,
including someone like Aaron King, treat discrete counts using continuous
distributions (but also adding some clever things on top of it so that
it's not totally crazy to keep cont distributions). Maybe it's better to say
that we want to respect the data structure? At the end, we're just making
assumptions about what the observation error distribution might be...}
For example, we could
assume a Poisson error distribution with mean equal to the fitted
model trajectory [equation~\eqref{eq:SIR;R}]. This is convenient
because it does not require an extra parameter to be estimated.
But typical observation errors are \term{overdispersed}, meaning that
the variance greater than the mean (the variance is equal to the mean
for a Poisson distribution), and ignoring the overdispersion can
lead to underestimation of uncertainty. The negative binomial distribution can
be considered an overdispersed Poisson, which makes it a good choice \citep{linden2011using}.


The probability mass function for the negative binomial distribution
(for counts $k=0,1,2,\ldots$) is
\begin{linenomath*}
\begin{equation}
...
\end{subequations}
\end{linenomath*}
We can again get the CIs by taking square roots and computing
