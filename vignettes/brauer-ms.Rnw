...
To do better, it will help to think about the probability of observing
data $\{y_i\}$ assuming normal errors as above, i.e.,
%%
\begin{linenomath*}
...
\exp\left(-\frac{\big(f(t_i;\betavec) - y_i\big)^2}{2\sigma^2} \right) \Delta{y_i}
\right]
\,,
\end{equation}
\end{linenomath*}
...
\emph{proposed} model is, given the data that \emph{were}
observed. One way to get at this is to use Bayes rule for conditional
...
\lik(\betavec)
= \Pop (\{y_i\} \mid \betavec) \cdot \Pop(\betavec)\,.
\end{equation}
\end{linenomath*}
...
$\lik$ with respect to $\betavec$ or, equivalently, minimizing the
negative log-likelihood
\begin{linenomath*}
\begin{equation}\label{eq:negloglik}
\min_\betavec\big(\!\!-\log{\lik(\betavec)}\big) = \min_\betavec\Big( 
\sum_{i=1}^n \big(f(t_i;\betavec) - y_i\big)^2 
\;+\; \text{constant}
\Big)
\end{equation}
\end{linenomath*}
yields an estimate, say $\betavechat$, that---lo and behold---agrees
...
solution $\betavechat$ is the \term{maximum likelihood estimate} (MLE)
of $\betavec$, under the assumption of normal observation errors in
the time series.
...
parameters).  We will then end up with a different likelihood function
to maximize, and obtain a different $\betavechat$, but the basic idea
is the same.
...
and CIs on the estimated parameter values $\betavechat$?

Our data are actually counts of deaths (or cases in other contexts),
so the observation error distribution can't really be normal (and it
is the assumption of normality that leads to a lower limit of the
confidence band on the fitted model being negative when the time
series is near zero).  We could assume a Poisson error distribution
with mean equal to the fitted model trajectory
[equation~\eqref{eq:SIR;R}], but typical observations have variance
greater than the mean (which are equal for a Poisson distribution).
The negative binomial distribution can be considered an overdispersed
Poisson, which makes it a good choice.  \djde{Ben, please feel free to
  give a more principled justification, and to explain how the neg
  binom dispersion parameter would enter the likelihood function if we
  wanted to fit it.}

Regardless of the form of the likelihood function, we can use it to
obtain CIs on the MLE $\betavechat$.  A relatively simple approach is
to use the the shape of $\log{\lik(\betavec)}$ in the vicinity of
$\betavechat$ to infer parameter values of a multivariate normal
distribution for $\betavec$.  Near $\betavechat$, the shape of
$\log{\lik}$ is determined by its \term{Hessian matrix} (the matrix
...
is the \term{variance-covariance matrix} $\mathrm{Cov}(\betavec)$ that
specifies the desired multi-variate normal with mean $\betavechat$.
\djde{What is a simple way to explain why the inverse of the Hessian
  is the cov matrix for the parameters?}

From the multi-variate normal, we can calculate CIs on each of the
components of $\betavechat$ by\djde{explain\dots}.  Then to obtain CIs
...
$\Delta{g}\approx g'(a)\Delta{a}$.  Given a nonlinear function $g(\betavec)$
of the parameters, the \term{Delta Method}
\cite{Dorf38,VerH12} expands $\textrm{Var}(g(\betavec))$ to first
order about $\betavechat$, which gives us the variance-covariance matrix
of $g(\betavec)$ \cite{Bolk08}.  In particular, the variance of
$g(\betavec)$ is
\begin{linenomath*}
\begin{subequations}\label{eq:DeltaMethod}
\begin{align}
\mathrm{Var}(g(\betavec))
&\approx 
\mathrm{Var}\big[g(\betavechat) 
   + (\nabla g)(\betavechat)\cdot(\betavec-\betavechat)\big] \\
&=
\mathrm{Var}\big[(\nabla g)(\betavechat)\cdot(\betavec-\betavechat)\big] \\
&= 
\mathbb{E}\big[\big((\nabla g)(\betavechat)\cdot(\betavec-\betavechat)\big)^2\big] \\
&= 
\,\mathbb{E}\big[
(\nabla g)(\betavechat)^\transpose
  (\betavec-\betavechat)
  (\betavec-\betavechat)^\transpose
  (\nabla g)(\betavechat) 
  \big]\,
\\
&= 
...
\end{subequations}
\end{linenomath*}
We can then take the square root of the estimated variance to obtain the standard error ($\mathrm{SE}$) and calculate approximate confidence intervals by adding $\pm1.96\,\mathrm{SE}$ to $g(\betavechat)$.

Given a fit of KM's approximation \eqref{eq:sech} to the time series
data, which yields $\betavechat=(\hat{a},\hat{\omega},\hat{\phi})$, we
can apply the Delta method \eqref{eq:DeltaMethod} to the nonlinear
relationships \eqref{eq:invertparams} to obtain CIs on
$g(\betavechat)=(\hat{\Reff}, \hat{\gamma},\hat{S_0})$.  This is
...
considering $g(\betavec)=f(t_i;\betavec)$.  This is how we obtained
the pink confidence band shown in Figure~\ref{fig:Bombay}.

\djde{Available in \code{fitode} but we not what we doing here:
  Another posibility would be to use the estimated means and variances
  for $\betavec$ to define a multivariate normal, sample that many
...
gradient $(\nabla g)(\betavec)$ is not straightforward if $g$ is a
...
\cite{raue2013lessons}.  \djde{We will need to explain what the
  sensitivity equations are, at least in a footnote.}
...
\begin{linenomath*}
\begin{equation}
\sigma(t)=\sqrt{\frac{1}{n-1}\sum_{i=1}^n \big(f(t_i;\betavec) - y_i\big)^2} \,.
\end{equation}
\end{linenomath*}
\item[\code{initial}]conditions are expressed a numbers of individuals.
