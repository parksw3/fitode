...
\swp{This is also broken. Having trouble calculating vcov... so we can't get CIs for ols fits.}
\djde{``This'' is referring to \texttt{ci.bombay.fitode.ols}
  in the \texttt{sirmortfitcoefols} chunk, which is suppressed here.}
\bmb{Do we even expect to get CIs for OLS fits? I thought we explained above that OLS is not a statistical framework. If we want CIs we can either use a Normal error response (probably easiest), or back-calculate estimated resid SD from residual SSQs, scale Hessian appropriately ...}
\swp{The ols fits in the fitode already know how to back-calculate estimated resid SD and scale Hessian.
It's just that these fits are having problems calculating the Hessians in the first place...}
\bmb{We can comment basically that there is a bunch of unidentifability
and this is hard.  See footnote.  We could show likelihood profiles in
an appendix.}
