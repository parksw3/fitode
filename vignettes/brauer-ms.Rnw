...
\eqref{eq:sech} fails once the \hyperlink{KMassumption}{requirement}
that $\R_0 R(t)/N\ll1$ is violated.  Nonetheless, we can still fit the
\KM approximation to the simulated epidemic and obtain visually
reasonable agreement (\cref{fig:stoch}, bottom panel, red curve;
\cref{tab:stoch.nls}); the fit cannot get better because the
approximation \eqref{eq:sech} is symmetric about the peak, whereas the
rise is steeper than the fall in the true solution.  In addition, the
estimated parameters that yield the red curve are far from the true
parameters that were used in the simulation (\cref{tab:stoch.nls}).

The excellent fit of the determinsitic trajectory that \code{fitode}
