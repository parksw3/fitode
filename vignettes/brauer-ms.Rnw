...
The highest level of test is to begin by simulating data that are more
realistic than the idealized model that we want to fit is capable of
generating, and then see if we can nevertheless recover the underlying
parameter values that correspond to those of our idealized model
(e.g., the SIR model).
