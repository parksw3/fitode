...
\djde{Notation: $S_{\rm i}$ etc would be better than $S_0$ to further
  reduce potential confusion with $\Rn$.  I often do this.  Not sure
  why I didn't here.}
...
stochastic SIR model with initial state $(\Sinit,\Iinit,R_0)=(\Sexpr{iii})$,
...
\caption{\textbf{Deterministic fits to daily incidence generated by a
  ...
  $(\Sinit,\Iinit,R_0)=(1998,2,0)$, basic reproduction number $\Rn=5$, and
