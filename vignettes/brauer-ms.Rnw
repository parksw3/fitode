...
This is different from the likelihood
 associated with Normal errors, where $\sigma^2$ cab be either
...
\end{description}
The resulting fits are plotted in \cref{fig:Bombay}.
The estimated coefficients and the associated confidence intervals
(\cref{tab:bombay.fitode}) can be obtained via \code{coef} and \code{confint} functions;
the latter function also allows users to obtain confidence intervals for derived parameters using the Delta method.
Note that \code{fitode} gives discrete predictions (rather than smooth curves) because we are calculating mortality at discrete (weekly)
time intervals using \cref{eq:correct.mortality2}.
...
\begin{table}
  \begin{center}
    ...
    See \cref{fig:phila}.}\label{tab:philanls}
    \medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
  \begin{tabular}{ m{3cm} | c | c | c | c c}
