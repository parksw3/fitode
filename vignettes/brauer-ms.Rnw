...
$k$ together with the other parameters ($\Sinit,\Iinit,\R_0,\Tg$).
...
\begin{table}
  \begin{center}
    \caption{\textbf{Fits of \KM's \cite{KermMcKe27} analytical SIR
      ...
      generate the stochastic simulation ($\Sinit$, $\Iinit$, $\R_0$ and
...
\caption{\textbf{Deterministic fits to daily incidence generated by a
  ...
  $(\Sinit,\Iinit,R_0)=(1998,2,0)$, basic reproduction number $\R_0=5$, and
