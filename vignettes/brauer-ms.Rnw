...
\begin{abstract}
  ...
  systems, especially differential equations.
  He made many contributions to mathematical epidemiology, a field
  that is strongly connected to data, but he had no desire to touch
  data.  Nevertheless, he recognized that fitting models to data is
  usually necessary when attempting to apply infectious disease
  transmission models to real public health problems.  He was curious
  to know how one goes about fitting dynamical models to data, and why
  it can be hard.  Initially in response to Fred's questions, we
  developed a user-friendly \code{R} package, \code{fitode}, that
  facilitates fitting ordinary differential equations to observed time
  series.  Here, we use this package to provide a brief tutorial
  introduction to fitting compartmental epidemic models to a single
  observed time series.  We assume that, like Fred, the reader is
  familiar with dynamical systems from a mathematical perspective, but
  has limited experience (if any) with statistical methodology or
...
\section*{Remaining items on ToDo list}
...
\item statistical challenges, where to go for further insights / more
  advanced approaches
\item fitting ABMs
\item \bmb{Note that Baca\"er \cite[Table~2, p.\.412]{bacaermodel2012}
...
\item \bmb{pedagogical refs: I would use Raue et al
    \cite{raue2013lessons} for sensitivity equations and see how much
    of the rest is defined in Bjornstad's book
    \cite{bjornstadEpidemics2018} (or fall back on mine).}
\end{itemize}
...
susceptible, infected or removed\footnote{In the words of \KM
  ...
  are sick, by recovery or by death''. \bmb{is this needed?}\djde{No,
    but I like it.  I didn't realize until finding this quote that KM
    themselves thought of $R$ as a class for both recovered or dead
    individuals.}}, $\beta$ is the transmission rate, and $\gamma$ is
...
\djde{Perhaps add a footnote mentioning Ben's discovery that including
gradients in the fit eliminates this symmetric solution?}
...
multiple local optima makes fitting to data hard; you need to
...
taken into account.
\bmb{I wrote some stuff (commented out here), but then realized that it's
  pretty well covered by the caveats paragraph from KM at the end of this
  section (search for ``mental gymnastics'').
  The Ellner fit I was referring to is unpublished, \href{https://ms.mcmaster.ca/~bolker/eeid/2010/Ecology/EllnerMaxLikEEID2010.pdf}{here}}
\djde{I've temporarily exposed your text in grey to avoid commented-out stuff in the ms.}
{\color{gray}
(If we think carefully about the biology we 
realize that we are sweeping many more assumptions under the rug:
for example,
in reality plague is a vector-borne disease of rodents that spills over
into humans, rarely being transmitted directly between humans. One
way to gloss over these complexities is to say that $\beta$ and $\gamma$
implicitly both include a scaling factor that captures the relationship
between rat infections and observed human deaths.}
...
respect to the parameters, \ie
$\ddt{}\big( \gradtheta\xvec(t,\thetavec) \big)$.  They can easily be
derived using the chain rule; if we write a generic, autonomous ODE as
$\dbydt{\xvec} = \fvec(\xvec,\thetavec)$ then the \term{sensitivities}
are \djde{using \macro{sens} (currently $\sens$) rather than $S$ to
  avoid confusion with susceptibles}
\begin{linenomath*}
\begin{subequations}
\begin{align}
\sensmat(\xvec,t, \thetavec) 
...
&=  \gradx\fvec(\xvec,\thetavec)
    \, \gradtheta\xvec
   +  \gradtheta\fvec \,. \label{eq:sens;b}
\end{align}
\end{subequations}
\end{linenomath*}
\djde{I'm assuming an autonomous ODE; not clear what $\gradtheta$
  means otherwise.}\djde{\cref{eq:sens;b} requires us to know
  $\gradtheta\xvec$, but we don't.  So how do we compute
  this?}\djde{It seems to me that the notation should be
  $\sensmat(\xvec(t,\thetavec),\thetavec)$ to make clear that there is
  no explicit dependence on $t$.}From the time-varying matrix
$\sensmat$, we can calculate the derivative of $x_i(t,\thetavec)$ with
respect to $\theta_j$ as
$\int_0^{t} \sens_{ij}(\xvec,t',\thetavec) \, {\rm d}t'$, \ie by
integrating along the solution to the differential equation; we can
then use a further chain-rule step to compute the derivative of the
log-likelihood of the observations at time $t$ with respect to the
parameters. Integrating the sensitivity equations in parallel with the
ODEs is a computationally efficient and numerically stable way to
calculate the overall gradients of the log-likelihood with respect to
the parameters, which makes nonlinear estimation more robust and
...
\bmb{Ugly: \url{https://stackoverflow.com/questions/48328364/inline-code-format-with-knitr-and-rnw}; \url{https://stackoverflow.com/questions/40252885/how-to-syntax-highlight-inline-r-code-in-r-markdown}?}
  %%
...
Taking our previous parameter estimate from \code{nls}
...
explicitly made a sequence of optimistic assumptions, which they
...
particular, $\R_0\approx6.4$ and $\Tg\approx4.27$ days.
\bmb{we could import these numbers from R code (by writing to an
  included TeX file with definitions, or by using inline expressions)
  rather than hard-coding them, if we wanted ...}
\djde{Yes, we should do that to reduce the chance of screwing up
  if we end up changing things.}
...
Even with these extensions, modelers will face many challenges when
...
the model can also help \cite{raue2013lessons,Bolk+2013}.  We also
...
As its name suggests, \code{fitode} is limited to fitting
ODEs to time series.  Consequently, by design, \code{fitode} ignores
\term{process error}, i.e., random variability that affects
both current and future steps of the trajectory---as opposed to \term{observation error}, which
arises from imperfect measurements or reporting and is usually assumed to
be independent of the trajectory itself.  A key component of
process error is the demographic stochasticity that is inherent to the
discrete-state stochastic SIR model discussed above (and to any real host-pathogen
system).  Parameters of models can also be subject to process error;
for example, the transmission rate might depend on random fluctuations in weather.
Properly accounting for process error can be
critical for estimating uncertainties in parameter estimates
and confidence bands on the projected dynamics of a system \citep{King+15,taylor2016stochasticity,li2018fitting}.
Popular \code{R} packages that can fit models with process error
include \code{pomp} \citep{king2015statistical} and \code{odin}
\citep{odin2021}.
\bmb{can odin do that? really?}
...
\begin{table}
  \begin{center}
    \caption{Parameters of \KM's \cite{KermMcKe27} approximation
      ...
      them \cite[p.\,714]{KermMcKe27} and by us [using nonlinear least
      squares (\code{nls})].  Values for the initial prevalence $I_0$
      and population size $N$ are assumed in order to derive estimates
      of the standard SIR model parameters from the parameters of
      \KM's approximation (using the indicated equations).  Like
      Baca\"er \cite[p.\,408]{bacaermodel2012}, we assume the
      population of Bombay was $N=1$ million.  \djde{I revised this
        caption and changed the table order to
        Estimated-Assumed-Derived, which is more logical.}
    }\label{tab:Bombay}
\medskip
...
\begin{table}
  \begin{center}
    \caption{Parameters of the SIR model \eqref{eq:SIR}, as estimated
      using \code{fitode} to fit numerically computed trajectories to
      the Bombay plague data, assuming observation errors are
      distributed negative binomially (\code{dnbinom}) or normally
      (\code{ols}).  \djde{Why are there no CIs for \code{ols}?}  The
      recovery rate $\gamma$ was fixed rather than fitted due to
      parameter unidentifiability.  \djde{Can we say what goes wrong
        if one attempts to fit $\gamma$ as well?  And why fix $\gamma$
        rather than some other parameter?  Such info could be given in
        footnote 9, which occurs in the explanation of the
        \code{fixed} argument in the main text.}  \djde{We should
        state why $\gamma$ was fixed at 4.11 specifically.}
      %%
      The fitted trajectory and
      confidence band are shown in the lower panel
      of \cref{fig:Bombay}.
      %%
      As in \cref{tab:Bombay}, a population size $N$ must
      be assumed to derive $\R_0$ estimates.
...
points(mort ~ week, data = fitode::bombay, xpd=NA, pch=21, bg="white", lwd=2)
legend("topleft", bty="n", lwd=c(2,lwd*0.60,lwd),
       lty=c(NA,"dotted","solid"),
       col=c("black",col.nls,col.fitode.nb),
       pch=c(21,NA,NA),
       pt.bg=c("white",NA,NA),
       legend=c("observed data","our nls fit","fitode (nbinom)"))
@
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July
  ...
  distributed; see \cref{sec:Uncertainty}).  
  \djde{The closeness of the \KM fit and the \code{fitode} fit
    is remarkable.  Perhaps \KM had a time machine and actually
    went 100 years into the future to use \code{fitode}.}
  %%
  \emph{Bottom panel:} The yellow curve is identical to the yellow
  dotted curve in the top panel\djde{No, in the top panel the yellow
    dotted curve is smooth, not interpolated from values at
    observation times.} \swp{I'm confused because they are identical?
    They're both using the same code too. See L2143 and
    L2157}\djde{Perhaps my eyes are deceiving me, but the yellow dots
    in the upper panel to not appear to be consistent with the solid
    yellow curve in the bottom panel.}, and the yellow band shows the
  \code{fitode} confidence band; this confidence band is shown as a
  linear interpolation between successive observation times because
  the model fits to incidence at discrete time points rather than to a
  continuous curve representation of the instantaneous death rate.
  The dotted red curve is identical to the solid red curve in the top
  panel.  We have separated the two panels because the confidence band
  overlap would make the plots difficult to interpret.  \djde{In the
    bottom panel, why don't we also show the \code{fitode} \code{ols}
    fit, for which we list estimated parameters in
    \cref{tab:bombay.fitode}?}  }
\label{fig:Bombay}
\end{figure}
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates for the 1918 influenza epidemic in
      ...
      during the main wave of the pandemic.  In order to derive estimates
      of the standard epidemiological parameters, we assume the
      initial prevalence had the value estimated by \code{fitode} (see
      \cref{tab:phila.fitode}) and the population size was the known
      population of Philadelphia at the time.  The fitted trajectory
      and confidence band are shown in \cref{fig:phila}.  
      See \cref{sec:phila}.
      %% 
      \djde{I filled in \code{philapop1918} there.  We also assumed
        ...
        anything.  Should we make more of this explicit?}  \swp{Ah yes
        we should make this more explicit. Shouldn't we also fill out
        \code{philaS0} for R0 estimates instead o f
        \code{philapop1918}? As far as I can tell, we're using
        \code{philaS0} to get R0 estimates? For example, see L1359 and
        L1399.}  }\label{tab:philanls}
    \medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
  \begin{tabular}{ m{3cm} | c | c | c | c c}
    ...
    population size & $N$ & -- & -- &
      \Sexpr{prettyNum(philapop1918, big.mark=",")}& -- \\
    \noalign{\vspace{10pt}}
    \bfseries Derived\break parameter \\\hline
    peak time & $\tpeak$ & \eqref{eq:tpeak} & weeks &
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      \djde{Daniel, did you fix $\gamma$ for this fit?  I assume so
        from the lack of a CI, but then it should be listed as a fixed
        parameter, not an estimated parameter.  Where did this fixed
        value come from?  I'm then confused
        by the CI on $\Tg$, which just have no error since it is just
        the inverse of a fixed parameter.}
      As in \cref{tab:philanls}, we assume the known 1918 Philadelphia
      population size in order to derive an $\R_0$ estimate.
...
\caption{The main wave of the 1918 influenza epidemic in the city of
  ...
  confidence band
  show a nonlinear least squares (\code{nls}) fit of \KM's
  approximation \eqref{eq:sech}; the parameter estimates are given in
  \cref{tab:philanls}.  The solid yellow curve and yellow confidence
  band show the \code{fitode} fit, for which the parameter estimates
  are given in \cref{tab:phila.fitode}.  The dotted yellow curve shows the \KM
  approximation using the parameters estimated with \code{fitode}.
  \bmb{I don't see a blue curve? (orange?)}
  \djde{I've changed the colours and line styles in a way that makes
    more sense to me... hopefully others agree... I've adjusted
...
\begin{table}
  \begin{center}
    ...
    \djde{I don't understand that.  What does the KM column
      ...
      (\code{nls}).
      \djde{This table is very confusing.  The true values of the stochastic
      SIR simulation parameters are listed under ``Derived
      parameter''.  I understand why this was done, but it is very confusing.
      Also, true values of the KM approximation parameters
      can be given and should be given rather than having ``--'' in those
      slots, but these values are in the KM column.  Again, I can see
      why this was done, but it is confusing.  I need to think more about
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      $\R_0=\Sexpr{R0}$, $\Tg=\Sexpr{1/mm$gamma}$ week.
      \djde{It would be helpful to have a \textbf{true} column
      between \textbf{units} and \code{dnbinom}.}
...
@
\caption{Deterministic fits to daily incidence generated by a
  stochastic SIR simulation with initial state
  $(S_0,I_0,R_0)=(1998,2,0)$, basic reproduction number $\R_0=5$, and
  mean generation interval $\Tg=1$ week.  \djde{Daniel, did you
    observe the simulation with a negative binomial process?  (if so,
    what $k$?)  Or did you just count the number of new cases each
    day?}
  %%
  In both panels, the yellow curve and confidence band show the
  \code{fitode} fit to the simulated data.  \emph{Top panel:} The
  solid green curve shows the solution of deterministic SIR model \eqref{eq:SIR}
  with the initial conditions and parameters used for the stochastic simulation.
  The dotted green curve shows the \KM approximation \eqref{eq:sech}
  to this deterministic trajectory.  The time shift between the
  green and yellow curves arises because there is a random delay
  until the stochastic trajectory begins to grow exponentially.
  %%
  ...
  \djde{Why are we not showing a confidence band for the \code{nls} fit?}
  ...
  %%
  \djde{Ben, I've retained the following comment.  Not sure
...
\caption{\textbf{Unidentifiability of the mean generation interval
  ...
  parameter unidentifiability.  \djde{Can you please change the style
    of the $y$-axis tick labels in panels B and C?  \ie
    $9.23\times10^{-5}$ rather than {\upshape\code{9.23e-05}}?  An
    alternative is to write $9.23$ and put $\times10^{-5}$ at the top
    edge of the $y$-axis.}  }\label{fig:proflikgamma}
\end{figure}
