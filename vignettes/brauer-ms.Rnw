...
\djde{The issue is that we follow \KM and present a formula for
  $\dbyd{R}{t}$ rather than $I(t)$.  In practice, is is the prevalence
...
model is
\djde{Why {\upshape\textbf{Prob}} rather than $\Pop$?
and why $dx$ rather than $\Delta x$?
and why is there a $d{\traj[\ti]}$ in the second line?
I've added a missing $+dx$ in the first line (and $\le$ instead of $<$ on the left), and reformated a bit.}
%%
\begin{linenomath*}
  \begin{equation}\label{eq:prob.data.given.model}
    \begin{split}
      \Pop(\text{data} \mid \text{model})
      & = \lim_{dx \to 0} \frac{\textbf{Prob}\left(\traj(\ti;\theta) \le \traj[\ti] < \traj(\ti;\theta) + dx \right)}{dx} \\
      & = \prod_{\tindex=1}^n
