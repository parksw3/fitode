...
the removal (or recovery) rate.  In that original paper, \KM
...
probability of observing the data $\{\trajobsti\}$ given the ODE trajectory 
$\{\traj(\ti; \thetavec)\}$.
\djde{The ODE trajectory is $\traj(t; \thetavec)$, not $\{\traj(\ti; \thetavec)\}$.
  Do statisticians think of $\{\traj(\ti; \thetavec)\}$ as the model
  in this context?  I think of $\traj(t; \thetavec)$ as the model.}
We imagine that the
...
and the observed data).  Then the probability density $\pdens$ of the data given the
model is
%%
\begin{linenomath*}
  \begin{equation}\label{eq:prob.data.given.model}
    \begin{split}
      ...
      & = \lim_{\Delta x \to 0} \frac{\Prob\big(\traj(\ti;\thetavec) \le \trajobsti < \traj(\ti;\thetavec) + \Delta x \big)}{\Delta x} \\
      & = \prod_{\tindex=1}^n
...
\end{linenomath*}
%%
(Note that we write $\Prob$ for the probability measure and $\pdens$ for
the probability density.)  We use a probability density function here
because the Normal is a continuous distribution; we would use a
probability mass function for a discrete response distribution such as
the Poisson. In practice, we don't have to worry about this
distinction when we are estimating the parameters of an epidemic model
(the increment $\Delta x$ will always appear as a constant multiplier
or divisor and doesn't affect any of our conclusions). 
