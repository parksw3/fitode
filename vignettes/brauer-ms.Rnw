...
To this point, we have addressed only an optimization problem, which
we solved using the method of nonlinear least squares.
\swp{But it is important to recognize that parameter estimates are never certain.
Uncertainties in parameter estimates can be caused by errors in the data
as well as model misspecification. And some of these uncertainties
can be captured by \term{confidence intervals} (CIs).
}
For example, the final
column of Table~\ref{tab:Bombay} lists 95\% CIs on our \code{nls}
parameter estimates, and the
\nlscol shaded region in Figure~\ref{fig:Bombay} is a \term{confidence
  band} for the fitted model curve.  \djde{currently it is actually
  the fitode confidence band, and is \fitodecol.}  Estimating levels
of \emph{confidence} \swp{uncertainty?} in our results requires some statistical
thinking.
\swp{do we want to define what confidence intervals mean?}
...
We estimated our CIs using a standard approach.  We imagine that the
...
\swp{Here's my stab at it:}
 However, this method of drawing confidence bands ignore uncertainties
associated with parameter estimates.
In doing so, we need to first estimate the \term{variance covariance matrix} for our parameter estimates
...
\swp{do we have to mention that profile likelihoods are a thing but are
...
\item[\code{observation}]specifies that the observed data
  ...
  equation~\eqref{eq:SIR;R}) and the standard deviation is $\sigma(t)=$\dots
\item[\code{initial}]conditions are expressed as proportions of the
  total population size (\code{N}),
\item[\code{par}]refers to the parameters that are to be fitted:
  $\beta$, $\gamma$, and initial proportions $s_0$ and $i_0$, which
  are to fitted via a
\item[\code{link}]function to the logit scale, since they are
  proportions; log links are assumed by default, so $\beta$ and
  $\gamma$ are constrained to be positive automatically.
\end{description}

\swp{I wrote this without realizing that you had already written the thing above. See if you need anything from here and keep it (or take it out). Not sure how much we want to explain the syntax though.}
\swp{Here, the gradients are defined using the \code{model} argument (using a list of formulas), where the left-hand side (LHS) corresponds to the name of the state variable (e.g,. \code{S} for $dS/dt$) and the right-hand side (RHS) the gradient.
The \code{observation} argument takes a list of formulas, where the LHS correspond to the data and the RHS correspond to the observation (error) distribution and its parameters---here we write \code{ols(mean = gamma * I)} to perform ordinary least squares where the modeled mean corresponds to $dR/dt=\gamma I$.
Similarly, the \code{initial} argument takes the initial values for the mode.
Finally, all model parameters (including initial conditions and parameters of the error distribution) must be listed using the \code{par} argument.
We note that log links are applied to all model parameters by default---this can be changed using the \code{link} argument as we show later.}
\swp{Do we want to move away from least squares here or not yet?}
...
sir.stoch.data <- sir.stoch[,c("time","I")]
names(sir.stoch.data) <- c("time", "mort")
...
lines(sir.det$tau, sir.det$I * N, col=my.red, lwd=3)
lines(sir.stoch$time, sir.stoch$I, lwd=2)
f <- function(t) KM_approx(t, a=a.det, omega=omega.det, phi=phi.det)
ttt <- seq(min(tt),max(tt),length=1000)
fff <- f(ttt)
lines(ttt,fff, col=my.blue, lwd=2)
lines(estimate ~ times, data=sss, col=my.green, lwd=2)
@
...
<<read.phila.data>>=
phila1918$time <- lubridate::decimal_date(phila1918$date)
@
...
start.phila <- c(beta=1e-4, gamma=1,
                 S0=philaS0,
                 I0=3)
names(start.phila) <- c("beta","gamma","S0","I0")
print(start.phila)
##
phila_fit <- fitode(
    SIR_model,
...
col.fitodeCI <- transparent_colour(col.fitode, alpha=0.4)
setup_plot <- function(ymax = 1000, xlab="", ylab="deaths", ...) {
    plot(NA, NA , bty="L", type="n", ylim=c(0,ymax),
         xaxs="i", yaxs="i", las=1,
         yaxt="n",
         xlab=xlab, ylab=ylab,
         ...
         )
    axis(side=2, at=100*(0:10), las=1)
}
...
draw_legend <- function(lwd=5, pt.bg="white") {
    legend("topleft", bty="n", lwd=c(2,lwd,lwd,lwd*0.60),
       ...
       col=c("black",col.KM,col.nls,col.fitode),
       pch=c(21,NA,NA,NA),
       pt.bg=c(pt.bg,NA,NA,NA),
       legend=c("observed data","KM","nls","fitode"))
}
@

\begin{figure}
<<Bombay-figure, echo=FALSE, fig.height=5>>=
f <- function(t) KM_approx(t, a=a.KM, omega=omega.KM, phi=phi.KM)
setup_plot(ymax = 900, xlim=c(0,tmax),
         xlab="weeks", ylab="plague deaths")
draw_confband(SIR_confband)
...
lines(estimate ~ times, data=SIR_confband, col=col.fitode, lwd=lwd*0.60, lty="dotted")
...
setup_plot(ymax = 800, xlim = range(phila1918$time), ylab="daily P&I deaths")
draw_confband(ppp)
tvals <- with(phila1918, seq(min(time),max(time), length=1000))
t0 <- min(tvals)
...
lines(tvals,
      ...
      xpd=NA, lwd=lwd, col=col.nls)
lines(estimate ~ times, data=ppp, col=col.fitode, lwd=lwd*0.60, lty="dotted")
