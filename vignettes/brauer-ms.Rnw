...
really follow a continuous, real-valued Normal distribution.  We could
assume a Poisson error distribution with mean equal to the fitted
model trajectory [equation~\eqref{eq:SIR;R}], but typical observations
have variance greater than the mean (the variance is equal to the mean
for a Poisson distribution).  The negative binomial distribution can
be considered an overdispersed Poisson, which makes it a good choice.
