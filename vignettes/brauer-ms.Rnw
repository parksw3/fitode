...
At this point, it is not surprising that we can find a set of parameters
for KM's approximation that matches the epidemic curve using
nonlinear least squares.
However, the estimated parameters are extreme, with an $\mathcal R_0$ of
2310 (Table~\ref{tab:philanls}).
\swp{Again, possibly something wrong with parameter conversions here too.
Haven't been able to figure out how to do it properly.}
The SIR model gives a visually similar fit as the
KM's approximation, while providing much more realistic parameter estimates
(Table~\ref{tab:phila.fitode}).
Converting the SIR parameters to KM parameters gives a worse fit, which
underestimates the size and timing of the peak.
\swp{Again, possibly something wrong.}
