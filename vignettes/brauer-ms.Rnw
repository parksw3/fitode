...
The red curve in Figure~\ref{fig:Bombay} does appear to provide a
...
column of Table~\ref{tab:Bombay} lists 95\% \term{confidence
  intervals} (CIs) on our \code{nls} parameter estimates, and the blue
shaded region in Figure~\ref{fig:Bombay} is a \term{confidence band}
for the fitted model curve.  Estimating levels of \emph{confidence} in
our results requires some statistical thinking.
