...
fit their model to plague mortality data from an epidemic in Bombay \swp{(now Mumbai)}
...
interest in \term{agent-based models}, which represent each
individual as a separate unit that can have unique properties \cite{eubank2004modelling}.
...
na\"{\i}ve \swp{simple?}, with little \swp{definitely too strong: or no} consideration of their quality; but over
...
their parameter estimates were obtained.  \djde{When was least-squares
  ...
  played with parameter values and fitted by eye?}
  \swp{It seems unlikely that they had the ability to perform least squares back in 1927... I was able to find least squares SIR and SI3R(!) model fit from 1990 \cite[p.\,51]{kryscio1990modeling} but not sure if there's anything earlier.}
Whatever their process, they must have engaged in some sort of \term{trajectory
...
Kermack and McKendrick (see Table~\ref{tab:Bombay}), and found that
the least squares parameter values differ from KM's by a few percent.
Our least squares fitted function is show in blue in
Figure~\ref{fig:Bombay}.
...
at the plotted observed data.  \djde{We could ``guess'' a value of
  $\omega$ by noting that $\sech^2{(\omega t)}\to e^{2\omega t}$ as
  $t\to-\infty$, so we could estimate $\omega$ as the slope of
  $\log(y_i/2)$\dots a little hard to do by eye from the data plot.}
...
If you experiment with initial guesses, you will find that if you make
a sufficiently bad guess, then \code{nls} will fail.  e.g.,
$a=2000$, $\tpeak=5$, $\omega=0.1$ yields
a \code{singular gradient} error.

More interesting is that
$a=500$, $\tpeak=5$, $\omega=0.1$ yields
$a=869$, $\omega=-0.19$, $\phi=-3.48$.
%% BMB: can we automate this some more?

This illustrates a very important fact: there is not necessarily a unique
best fit set of parameters!  In this case, this alternative solution
exists because $\sech^2$ is symmetric about the $y$ axis, but in
general, there can be many local minima that cause \code{nls} to
converge, and may not in general be equally good.

If you know that your parameters should be in a certain range, then you
can pass that information to \code{nls}.  For example, to ensure
that all the parameters are non-negative, you would add the option

<<lower, eval=FALSE>>=
lower = c(a = 0, omega = 0, phi = 0)
@

<<KM_fitted_values, echo=FALSE>>=
parnames <- c("a", "omega", "phi")
for (p in parnames) {
    assign(paste0(p, ".KM"), KM.parameters[[p]])
}
...
for (p in parnames) {
    assign(paste0(p, ".us"), fitted.parameters[[p]])
}
...
tpeak <- phi.us/omega.us
dig <- 3 # number of significant digits for printing
lines(tvals, KM_approx(tvals, a=a.us, omega=omega.us, phi=phi.us),
...
\djde{What is the standard way to get CIs on the parameters and confidence
  bands on the fit from the \code{nlsfit} object?} \bmb{\code{confint()} for CIs on parameters (now implemented). CIs on predictions are harder. \code{fitode()} has built-in methods, not very well documented in \code{help("predict,fitode-method")}; for \code{nls} the \code{propagate} package works, but is doing some pretty fancy stuff (simulation plus higher-than-second-order Taylor expansions, I think)}
...
\djde{What is the best way to invert the parameter relationships in
  ...
  we compute CIs for the derived parameters?  Delta method?} \bmb{some combination of Delta method and ??; see epidemics vignette}
...
\bmb{This comment? ``None of these assumptions are strictly fulfilled and consequently the numerical equation
can only be a very rough approximation. A close fit is not to be expected, and deductions
as to the actual values of the various constants should not be drawn. It may be said,
however, that the calculated curve, which implies that the rates did not vary during the
period of the epidemic, conforms roughly to the observed figure.''  As \cite{bacaermodel2012} points out,
the fit of the curve to the data does \emph{not} necessary imply that the rates were constant \ldots}
...
<<fmt_CI, echo=FALSE>>=
ci_fmt  <- function(p) (format(cc[p,], digits = 2)
    |> paste(collapse = ", ")
    |> sprintf(fmt = "(%s)")
)
@
...
\begin{table}
  ...
  \begin{tabular}{l | c | c | c | c c}
    ...
    peak removal rate & $a$ & 1/weeks & \Sexpr{a.KM} &
      \Sexpr{signif(a.us,dig)} & \Sexpr{ci_fmt("a")} \\
    outbreak speed & $\omega$ & 1/weeks &
      \Sexpr{omega.KM} & \Sexpr{signif(omega.us, dig-1)} &  \Sexpr{ci_fmt("omega")} \\
    outbreak centre & $\phi$ & -- & \Sexpr{phi.KM} &
      \Sexpr{signif(phi.us, dig)} &  \Sexpr{ci_fmt("phi")} \\
    \noalign{\vspace{10pt}}
