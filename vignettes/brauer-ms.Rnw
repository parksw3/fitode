...
the log-likelihood of the observations at time $t$ with respect to the
parameters.
\djde{We should write down that chain-rule step too.}
%%
Integrating the sensitivity equations in parallel with the ODEs is a
