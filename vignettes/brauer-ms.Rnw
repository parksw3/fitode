...
\hypertarget{KMassumption}{}and an exact analytical form for this
...
\KM column of \cref{tab:Bombay}, and they plotted their
...
soon).  The \KM approximation is good initially, but becomes poorer
and poorer as \hyperlink{KMassumption}{the underlying
  approximation}---that $\R_0$ times the proportion removed is
small---is less and less accurate.
\djde{It might be best to display the \KM assumption so we can
  refer to an equation here.  No journal will allow us to include a
  hyperlink to the location of an inline expression.  It isn't a
  completely trivial change because $\R_0$ isn't defined until
  after the \KM assumption is stated.}
...
stochastic processes in a population of $N$ individuals.  Realizations
of this discrete-state model can be generated exactly using the
Gillspie algorithm \cite{Gill76}, or approximately (as we do here)
using the ``$\tau$-leaping'' approach \cite{Gill01}, which is
implemented in the \code{adaptivetau} R package \cite{adaptivetau}.
The demographic stochasticity (aka \term{process noise}) that these
...
interval $\Tg=\gamma^{-1}=\Sexpr{1/mm$gamma}$ week.

\thickredline

Finally, we apply the same methodologies to
a simulated mortality curve.
In particular, we simulated a stochastic
SIR model using an adaptive tau algorithm\djde{with $N=2000$ we should
  be using Gillespie}, with $(S_0,I_0,R_0)=(\Sexpr{iii})$,
$\R_0=\Sexpr{R0}$,
infectious period ($\gamma^{-1}$) $=\Sexpr{1/mm$gamma}$ week.  \cref{fig:stoch} (top
panel) shows the simulated daily mortality curve (dots) alongside the
smooth, deterministic trajectory of epidemic mortality ($\dbydt{R}$,
green) using the same parameter values and initial conditions.  For
this particular stochastic realization, the epidemic takes off slowly,
resulting in a time shift between the deterministic and stochastic
trajectories.

In this case, \KM's approximation \eqref{eq:sech}---using the correct parameter values---yields a poor representation of the dynamics.
The approximation predicts a much lower epidemic peak and a shorter duration of the epidemic (\cref{fig:stoch}, top panel, blue curve).
Nonetheless, we can still fit the KM approximation to the simulated epidemic and obtain visually reasonable agreement (\cref{fig:stoch}, bottom panel, red curve; \cref{tab:stoch.nls}); the fit cannot get better because the approximation \eqref{eq:sech} is symmetric about the peak, whereas the rise is steeper than the fall in the true solution.
In addition, the estimated parameters are far from the true parameters that were used to simulate the epidemic (\cref{tab:stoch.nls}).

The exact deterministic SIR model gives a much better fit, which
is achieved by estimating an initial prevalence that is only a third of the true initial prevalence; all other parameter estimates are nearly identical to the true parameter values used to generate the stochastic trajectory (\cref{tab:stoch.fitode}).
...
In this study, we present a guideline for fitting ODE models
to epidemic time series data alongside a user-friendly \code{R}
package for beginners. In doing so, we compare
SIR model fits to KM's approximations
across three data sets and their resulting parameter estimates.
For the Bombay plague data, both models give similar fits and parameter
estimates.
For the Philadelphia flu data, the nonlinear least square fit of KM's approximation
give extreme parameters even though it provides a visually good fit.
Finally, fitting a deterministic SIR model to a stochastic simulation shows that
underlying epidemiological parameters, except for the initial conditions, can be recovered
with reasonable accuracy.

Despite historical connections to KM \cite{KermMcKe27}, it is unclear whether
the SIR model is appropriate for studying the Bombay plague
data. Instead, Baca\"er argued that the epidemic represents a
seasonal outbreak and therefore requires a change in transmission
rate over the course of an epidemic \cite{bacaermodel2012}.
\swp{what is our opinion?}
Here, we also find that the Bombay epidemic poses a computational issue
for fitting the SIR model.
In particular, the profile likelihood becomes extremely flat for high
values of $\gamma$, indicating parameter unidentifiability.

We have focused on fitting the SIR model in this paper for illustrative
purposes, but the \code{fitode} can be used for fitting many other
models.
For example, any extensions of the SIR model that include additional
compartments, such as the SEIR or SIRS models, can be easily set up.
\code{fitode} also allows for non-autonomous models, such as the
SIR model with sinusoidally forced transmission rate.

There are several features that are implemented in \code{fitode} but
are not covered in this paper.
First, \code{fitode} allows confidence intervals to be calculated
using importance sampling.
In this case, parameter sets are randomly drawn from a multivariate normal
distribution whose mean and the variance covariance matrix corresponds to
the maximum likelihood estimates and the estimated variance covariance matrix,
respectively;
then, each parameter set is weighted accordingly based on the the likelihood of their
epidemic trajectories and the sampling probability.
Generally, this method is expected to provide more robust confidence intervals
than the Delta method but computationally more costly.
\code{fitode} also allows for simultaneous fitting of multiple state variables
to multiple time series and Bayesian inference via Markov chain Monte Carlo.

Even with these extensions, there will be many other challenges to
fitting ODE models with the \code{fitode} package, as with many other software.
For example, it is often difficult to ensure that the model has converged
properly or reached its true maximum.
Performing optimization from multiple starting conditions and drawing
likelihood surfaces can help diagnose these issues.
Using other optimization methods or reparameterizing the model can
sometimes help resolve these issues as well.
We also encourage users to open any issues via the GitHub repository:
\url{https://github.com/parksw3/fitode}.

There are many other issues that are not covered
by the \code{fitode} package that are important and useful for making
statistical inference from epidemic time series.
In particular, the \code{fitode} package only allows for
fitting deterministic ODE models, meaning that
it does not account for \term{process error}, which
refers to the variability in the dynamics of the system;
several studies have shown that accounting for
the process error is critical to capturing the uncertainties
in parameter estimates and describing the dynamics of a system.
Popular \code{R} packages that allow for stochastic model fitting include \code{pomp} \citep{king2015statistical} and
\code{odin} \citep{odin2021}.
...
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July
  ...
  \djde{The legend should have dotted rather than solid yellow.} shows
  the \code{fitode} fit, for which the associated parameter estimates
  are given in \cref{tab:bombay.fitode}.  \emph{Bottom panel:} The
...
\begin{table}
  \begin{center}
    \caption{Parameters of \KM's \cite{KermMcKe27} approximation
      ...
      (\code{nls}).  The true parameter values used for
...
\caption{
\textbf{The profile likelihood of the SIR model for the Bombay epidemic across a wide range of values for $\gamma$.}
(A) The profile likelihood is calculated by fixing $\gamma$ to a given value as maximizing the likelihood by estimating all other parameters.
A flat profile-likelihood surface indicates parameter unidentifiability, meaning that we can obtain statistically indistinguishable fits across a wide range of values for $\gamma$.
(B--E) The corresponding best parameter estimates for a given value of $\gamma$.
Strong correlations across parameters drive parameter unidentifiability.
}
\end{figure}
