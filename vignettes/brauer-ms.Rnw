...
Gillespie algorithm \cite{Gill76}, or approximately (as we do here)
...
\cref{tab:stoch.nls}). The \code{nls} fit cannot be improved further
...
(\KM, \cite{KermMcKe27}) approximation \eqref{eq:sech} to the standard
SIR model \eqref{eq:SIR}.  We also explained how to fit solutions of
ordinary differential equations (ODEs) to a time series---using our R
package \code{fitode}---and obtain parameter estimates and confidence
intervals, regardless of whether exact solutions of the ODEs are
available.

\code{fitode} is flexible enough to handle most compartmental
epidemiological and ecological models, so we hope the package will be
useful for many readers, not only as a pedagogical tool but also to
fit models to novel data.  Potential applications abound.  We have
ourselves used the predecessor of \code{fitode} (\code{fitsir}) to
study music popularity \cite{Rosa+21}.
