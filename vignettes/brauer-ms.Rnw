...
their parameter estimates were obtained.  \djde{When was least-squares
  ...
  played with parameter values and fitted by eye?} \bmb{FWIW, K\&M say (after
  commenting on all of the assumptions involved: ``None of these assumptions are strictly fulfilled and consequently the numerical equation can only be a very rough approximation. A close fit is not to be expected, and deductions
as to the actual values of the various constants should not be drawn. It may be said,
however, that the calculated curve, which implies that the rates did not vary during the period of the epidemic, conforms roughly to the observed figures.'' The use of ``implies'' here is interesting/potentially dodgy; in fact as pointed out by \cite{bacaermodel2012}, other lines of eviedence suggest that the rates \emph{did} vary \ldots } Whatever their
...
nlsfit <- nls(mort ~ KM_approx(week-1, a, omega, phi), 
              data = fitode::bombay,
              start = KM.parameters)
...
## note adjusted t for starting at t=1 vs t=0
f <- function(t) KM_approx(t-1, a=a.KM, omega=omega.KM, phi=phi.KM)
@ 

\begin{figure}
<<Bombayfigure, echo=FALSE>>=
tmax <- 33
