...
\swp{It seems unlikely that they had the ability to perform least
  ...
  before KM.}  Whatever their process, they must have engaged in some
...
we solved using the method of nonlinear least squares.  But the final
column of Table~\ref{tab:Bombay} lists 95\% \term{confidence
  intervals} (CIs) on our \code{nls} parameter estimates, and the
...
of \emph{confidence} in our results requires some statistical
thinking.
...
<<read.phila.data>>=
philadatafile <- "pim_us_phila_city_1918_dy.csv"
philadata <- read.csv(philadatafile)
philadata$date <- as.Date(philadata$date)
names(philadata) <- c("date","mort")
philadata$time <- lubridate::decimal_date(philadata$date)
@ 
...
setup_plot(ymax = 800, xlim = range(philadata$time), ylab="daily P&I deaths")
draw_confband(ppp)
tvals <- with(philadata, seq(min(time),max(time), length=1000))
t0 <- min(tvals)
lines(tvals, f(tvals-t0), lwd=lwd, xpd=NA, col=col.KM)
lines(estimate ~ times, data=ppp, col=col.fitode, lwd=lwd*0.60, lty="dotted")
points(mort ~ time, data = philadata, xpd=NA, pch=21, bg="white", lwd=2)
draw_legend()
@
