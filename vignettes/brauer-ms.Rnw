...
$\traj(\cdot\,;\cdot)$ when referring to the model.)  Minimizing
...
probability of observing the data $\{\traj[\ti]\}$.  We imagine that the
...
\exp\left(-\frac{\big(\traj(\ti;\thetavec) - \traj[\ti]\big)^2}{2\sigma^2} \right) \Delta{\traj[\ti]}
\right]
\,.
\end{equation}
\end{linenomath*}
...
model trajectory [\cref{eq:SIR;R} or \cref{eq:sech}]. This handles
...
\item [fit to multiple data streams:] \code{fitode} is not
  ...
  \djde{Maybe point out that our derivation of the sensitivity equations
  \eqref{eq:senseqns} accounts for multiple data streams.}
