...
&= \argmin_\thetavec 
               \sum_{i=1}^n \Big(\!\!
               -\log\Gamma\big(y_i+r_i(\thetavec)\big)
               +\log\Gamma\big(r_i(\thetavec)\big)
               -y_i \log\big(1-p_i(\thetavec)\big)
               -r_i(\thetavec)\log{p_i(\thetavec)}
               \Big)
 \,, \\
p_i(\thetavec) &= \frac{\sigma^2-f(t_i;\thetavec)}{\sigma^2} \\
r_i(\thetavec) &= \frac{f(t_i;\thetavec)^2}{\sigma^2-f(t_i;\thetavec)}
\end{align}
\end{subequations}
\end{linenomath*}
...
This fact is, admittedly, not obvious!  See \citep[\S6.5]{Bolk08} for
a heuristic explanation or \cite{Wassermanall2010} for a rigorous (if
terse) explanation.\djde{It would be nice to be more self-containedt
han this, if we can think of a useful way of doing so.}
