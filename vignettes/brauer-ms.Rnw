...
gradient $(\gradtheta g)(\thetavec)$ is not straightforward if $g$ is a
...
derivatives of the state variables with respect to the parameters, \ie
...
$\dbydt{\xvec} = \fvec(\xvec,\thetavec)$ then \djde{using 
  \macro{sens} (currently $\sens$)
  rather than $S$ to avoid confusion with susceptibles}
\begin{linenomath*}
\begin{subequations}
\begin{align}
...
\djde{I'm assuming an autonomous ODE; not clear what $\gradtheta$
  means otherwise} then we can calculate the derivative of $x_i(t)$
with respect to $\theta_j$ as
$\int_0^{t} \sens_{ij}(\xvec,t',\thetavec) \, dt'$, i.e. by solving the
differential equation; we can then use a further chain-rule step to
compute the derivative of the log-likelihood of the observations at
time $t$ with respect to the parameters. Integrating the sensitivity
equations in parallel with the main set of ODEs is a computationally
...
\emph{et al.} \cite{raue2013lessons} give a more detailed comparison
...
\item[\code{observation}]specifies that the observed data
  ...
  overdispersion parameter $k$; Ordinary least squares can be implemented by changing the \code{observation} argument to
  {\tt
    \hlstd{mort}
    \!\!{\color{black}\~{}}\!\!
    \hlkwd{ols}\hlstd{(}\hlkwc{mean} \hlstd{= R)}
}.
  % Ugly: https://stackoverflow.com/questions/48328364/inline-code-format-with-knitr-and-rnw; https://stackoverflow.com/questions/40252885/how-to-syntax-highlight-inline-r-code-in-r-markdown ?
...
want to fit, and then seeing if we can nevertheless recover parameters
...
\KM's approximation estimates the simulation
...
or mechanistically.  Nore generally, a purely
...
\item [fit to multiple data streams:] \code{fitode} is not
  ...
  can build a model that includes state variables for hospitalized individuals
  and for virus concentrations in wastewater, we can fit its parameters
...
\item [apply Bayesian inference:]
  ...
  \code{fitode} allows the user to specify prior distributions on parameters; these
...
discrete stochastic SIR model discussed above (and to any real host-pathogen
