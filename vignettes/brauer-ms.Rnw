...
Here, the gradients are defined using the \code{model} argument (using a list of formulas), where the left-hand side (LHS) corresponds to the name of the state variable (e.g,. \code{S} for $dS/dt$) and the right-hand side (RHS) the gradient.
