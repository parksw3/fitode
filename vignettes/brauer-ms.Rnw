...
recent years, as the power of computers has grown, there has been
increasing interest in \term{agent-based models}, which represent each
...
\djde{Intuitively at least, fitting $\tpeak$ or $\phi$ is likely to be
  ...
  commenting on?}
\bmb{I'm not sure it fits in here}
