...
$t$.''  Since (non-human) computers did not yet exist \citep{Camp09},
...
in \cref{eq:sech} can be written\footnote{There is a typographical
  error in equation (31) of \KM \cite{KermMcKe27}: their factor
...
\citep{Roge20}.  These data are exceptional because they are restricted
...
a deterministic model \citep{Earn+20}.)
...
\item [specify priors and apply Bayesian inference:] Unlike maximum
  ...
  (encapsulated in \emph{prior distributions}) as well as the observed
  ...
  disease system, or can be used to \emph{regularize} a fitting
  ...
  Bayesian modelers typically use \emph{Markov chain Monte Carlo}
  algorithms to explore the parameter space and approximate the target
  distribution. \code{fitode} implements a simple
  \emph{Metropolis-Hasting} sampler \citep[\S7.3.1]{Bolk08}.  (The
...
maximum.  More generally, naive and optimistic epidemic modelers often
...
small, noisy data set \citep{Gall+2022, Chow+2023}) when they first
start attempting to fit models to data.  In addition to the rigorous
...
\caption{\textbf{The plague epidemic in Bombay}, 17 December 1905 to 21 July
  ...
  (dots) were digitized from \citet[Table~IX, p.\,753]{jhyg1907}.
...
\begin{table}
  \begin{center}
    \caption{\textbf{Fits of \KM's \cite{KermMcKe27} analytical SIR
      ...
      CFP of $\Sexpr{philacfp}$ \citep{TaubMore06}.
      %%
