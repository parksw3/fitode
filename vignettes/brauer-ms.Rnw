...
%% observed values
\newcommand{\trajobssymbol}{\traj_{{\rm{\color{red}\scalebox{0.5}{obs}}}}}
\newcommand{\trajobs}[1]{\trajobssymbol\!\left(#1\right)}
\newcommand{\trajobsti}{\trajobs{\ti}}
%%\newcommand{\trajobsiti}{{\trajobssymbol}_{,\xindex}(\ti)}
\newcommand{\trajobsi}{\traj_{{\rm{\color{red}\scalebox{0.5}{obs}}},\xindex}}
\newcommand{\trajobsiti}{\trajobsi(\ti)}
\newcommand{\xvecobssymbol}{\xvec_{{\rm{\color{red}\scalebox{0.5}{obs}}}}}
...
\djde{I clarified total vs \emph{per capita} removal rates ($a$ vs
  $\gamma$), but we have ``recovery rate'' in a number of places.}
...
\end{equation}
\end{linenomath*}
In terms of $\Reff$, $\gamma$, $\Sinit$ and $\Iinit$,
...
\cref{eq:sechparams} can be inverted\footnote{Our expressions are
  ...
  expression for $Q$, the term $2Ry_0/x_0$ should be $2R^2y_0/x_0$; 
...
probability of observing the data $\{\trajobsti\}$ given the ODE trajectory $\{\traj(\ti; \thetavec)\}$.
We imagine that the
...
\begin{linenomath*}
  \begin{equation}\label{eq:prob.data.given.model}
    \begin{split}
      ...
      & = \lim_{\Delta x \to 0} \frac{\textrm{Prob}\big(\traj(\ti;\thetavec) \le \trajobsti < \traj(\ti;\thetavec) + \Delta x \big)}{\Delta x} \\
      & = \prod_{\tindex=1}^n
...
\end{linenomath*}
%%
\djde{We are now using $\Pop$ for both probability and probability density.
  hmmm.}
\bmb{what do you think about this solution?}
We use a probability density function here because the Normal is a continuous distribution; we would use a probability mass function for a discrete response distribution such as the Poisson. In practice, we don't have to worry about this distinction when we are estimating the parameters of an epidemic model (the infinitesimal $dx$ will always appear as a constant multiplier or divisor and doesn't affect any of our conclusions). In a mild abuse of notation, we will use $\Pop$ interchangeably for both probability  and probability density, referring to this quantity as``probability'' rather than ``probability density''.
...
$g(\thetavechat)=(\widehat{\Reff}, \widehat{\gamma},\widehat{\Sinit})$.  This is
...
the confidence band for the nonlinear least squares fit (light \nlscol) shown in \cref{fig:Bombay}.
...
\begin{align}
  ...
  &= \dd{}{\thetavec}\Big(\log\Pop\big(\{\xvecobsti:\tindex=1,\dots,\tindexmax\} \mid \xvec(\ti;\thetavec),\,\thetavec\big)\Big) \\
  &= \dd{}{\thetavec}\Big(\log\prod_{\tindex=1}^{\tindexmax} \Pop\big(\xvecobsti \mid \xvec(\ti;\thetavec),\,\thetavec\big)\Big) \\
  &= \dd{}{\thetavec}\sum_{\tindex=1}^{\tindexmax} \Big(\log \Pop\big(\xvecobsti \mid \xvec(\ti;\thetavec),\,\thetavec\big)\Big) \\
  &= \sum_{\tindex=1}^{\tindexmax} \dd{}{\thetavec}\Big(\log \Pop_\tindex(\xvec,\thetavec)\Big)
     \qquad [\text{\footnotesize abbreviating $\Pop_\tindex(\xvec,\thetavec) \equiv
       \Pop\big(\xvecobsti \mid \xvec(\ti;\thetavec),\,\thetavec\big)$}]
       \\
...
reproduction number $\Rn\approx \Sexpr{signif(R0.phila.nls,2)}$ and a
...
a fit---the solid \FOnbcol curve in \cref{fig:phila}---that is visually
similar to the (\nlscol) fit of \KM's approximation, but provides much
more realistic parameter estimates (\cref{tab:phila.fitode}); in
particular, $\Rn\approx \Sexpr{signif(R0.phila.fitode,2)}$ and $\Tg\approx
...
stochastic SIR model with initial state $(\Sinit,\Iinit,\Rinit)=(\Sexpr{iii})$,
basic reproduction number $\Rn=\Sexpr{R0}$, and mean generation
interval $\Tg=\gamma^{-1}=\Sexpr{1/mm[["gamma"]]}$ week.  In the top
panel, $\dbydt{R}$ [\cref{eq:SIR;R}] with the correct initial
conditions and parameter values is shown with solid \determcol, and the \KM
approximation \eqref{eq:sech} based on those parameter values is shown
with dotted \determcol.  The \code{fitode} fit [based on $\int (\dbydt{R})
\dt$] and confidence band are shown in \FOnbcol.  The time shift between
...
finds (\FOnbcol) is achieved by estimating an initial prevalence that is
...
is certainly debatable whether we can trust any inferences we might draw from
fitting the simple SIR model \eqref{eq:SIR} to these plague data.
As we quoted at the end of \cref{sec:fitode}, to justify
...
epidemics of plague
occurred in Bombay every year from 1897 to 1911 \cite[Fig.\,2]{bacaermodel2012},
suggesting
...
While \code{fitode} provides a relatively easy way to specify ODEs and estimate their parameters from data, any programming language will work to implement the steps we have outlined above, including both free general-purpose languages such as Python \citep{Bati+2022,Gupta2023} or commercial, domain-specific tools such as Matlab \citep{Chow2017} or Berkeley Madonna \citep{Zha+2020}. As long as a language provides tools for integrating arbitrary sets of ODEs (e.g. Matlab's \code{ode45}) and optimizing nonlinear functions (e.g. Matlab's \code{fminunc} or \code{lsqnonlin}), it can be used to estimate parameters of ODEs. However, \code{fitode}'s simple interface, automatic derivation of sensitivity equations, flexible specification of observation models, and provision of confidence intervals make it both convenient and powerful.
...
\item [specify priors and apply Bayesian inference:]
...
process error can be critical for accurately quantifying uncertainties in
parameter estimates and model forecasts
\citep{King+15,taylor2016stochasticity,li2018fitting};
however, the required statistical and computational procedures are
significantly more challenging than the approaches considered here.
...
\medskip
  ...
  \bfseries Estimated\break parameter & {\footnotesize\bfseries symbol}
    ...
    total removal rate at epidemic peak & $a$& \eqref{eq:a} & $\frac{1}{\textrm{weeks}}$ & \Sexpr{a.KM} &
...
\caption{\textbf{The plague epidemic in Bombay}, 17 December 1905 to 21 July
  ...
  approximation \eqref{eq:sech}, as fitted by \KM (\KMcol line) and by us
  using \code{nls} (\nlscol line, with confidence band estimated using the
...
\begin{figure}
  ...
  \caption{\textbf{The main wave of the 1918 influenza epidemic in the city of
    ...
    influenza (P\&I) are shown with dots.  The \nlscol curve and corresponding
...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{ m{3cm} | c | c | c | c | c c}
      ...
      \noalign{\vspace{10pt}}
      \bfseries Estimated\break parameter \\\hline
      total removal rate at epidemic peak & $a$& \eqref{eq:a} & $\frac{1}{\textrm{weeks}}$ & \Sexpr{signif(a.det,dig)} &
...
\begin{table}
  \begin{center}
    ...
    \begin{tabular}{ m{3cm} | c | c | c | c c}
      ...
      initial\break susceptibles & $\Sinit$& -- & \Sexpr{iii[[1]]} &
      \Sexpr{as.integer(S0.stoch.fitode)} & \Sexpr{ci_fmt("S0", ci.stoch.fitode, max_ord = 6)}\\
      initial prevalence & $\Iinit$ & -- & \Sexpr{iii[[2]]} &
      ...
      basic reproduction number & $\Rn$ & -- & \Sexpr{R0} &
