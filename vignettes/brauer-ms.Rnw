...
\min_\betavec\Big(\!-\log{\lik}\Big) = \min_\betavec\Big( 
\sum_{i=1}^n \big(f(t_i;\betavec) - y_i\big)^2 
\;+\; \text{constant}
\Big)
\end{equation}
...
Having developed the idea of maximum likelihood, we can do better than
the na\"ive approach by making a more realistic assumption about the
error distribution and/or taking advantage of any prior knowledge we
have that might affect our notion of the most likely parameter values
(i.e., \emph{not} ignoring $\Pop(\text{model})$, and instead
