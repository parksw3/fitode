...
particular, $\R_0\approx6.4$ and $\Tg\approx30$ days.  \djde{Ummm,
...
parameters of \KM's approximation, we obtain the blue \djde{orange?
  other colour?} curve in \cref{fig:phila}, which grossly
...
in \djde{colour??? do not use blue because the blue curve has a
  different meaning in \cref{fig:Bombay}.}.  The \code{fitode} fit and
...
\djde{We should have a ``true'' column in \cref{tab:stoch.nls} so the
  comparison we're suggesting is very easy to make.}
...
how to take advantage of each of these options?}
\begin{description}
...
\item[\term{Bayesian inference\needref via MCMC\needref:}]Markov chain
  Monte Carlo. \djde{BB or SWP: Can you add something about why this
    option is useful.}
\end{description}
...
on its own\needref, and estimating statistically meaningful
uncertainty in those optima is extremely challenging\needref.
...
\begin{table}
  ...
  \bfseries Assumed\break parameter & {\footnotesize\bfseries symbol}
    ...
    & \bfseries \code{dnbinom} & \bfseries 95\% CI & \bfseries \code{ols} \\\hline
...
\caption{The main wave of the 1918 influenza epidemic in the city of
  ...
  confidence band \djde{there is no pink confidence band visible}
  ...
  \djde{The yellow line should be dotted in the legend.}
  \djde{The KM curve has a completely different meaning from the
    KM curve in \cref{fig:Bombay} and should be labelled and coloured
    differently.  Perhaps labelled ``KM approximation'' and coloured orange,
    with this appearing below fitode in the legend.  We must be able to
    come up with better labels for the legends in all the figures.}
  \djde{The horizontal axis in this plot should have labels ``Sep'',
  ``Oct'', ``Nov'', etc, not 1918.75 etc.}
}
\label{fig:phila}
\end{figure}
...
\begin{table}
  \begin{center}
    ...
    \djde{I don't understand that.  What does the KM column
      ...
      (\code{nls}).
      \djde{The Estimated parameter section should be
      rounded to 3 significant digits.}
      \djde{$\beta$ should be given in units of 1/year.}
}\label{tab:stoch.nls}
    \medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
  ...
  \bfseries Assumed\break parameter & {\footnotesize\bfseries symbol}
    ...
    transmission rate & $\beta$ & \eqref{eq:beta} & $\frac{1}{\textrm{years}}$ & \Sexpr{52*R0*mm$gamma/2000} &
      \Sexpr{52*beta.KM.stoch} & \Sexpr{52*beta.nls.stoch} & \Sexpr{ci_fmt("beta", 52*cc.delta.stoch)} \\
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates from \code{fitode} fits of the SIR
      ...
      \djde{$\beta$ should be given in units 1/year.}
      \djde{$\Tg$ should be listed and given in days.}
      \djde{The upper limit of the CI for $k$ should not be in scientific notation.}
