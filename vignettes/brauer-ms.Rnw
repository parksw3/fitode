...
\djde{To be consistent with the way we've dealt with the KM approximation,
  we should probably set \code{fixed = c(N = 10^6)}, fit \code{s0}, 
  and set \code{r0 = 1 - (s0+i0)}.}

\noindent
In the model definition above, \code{model} specifies the vector field
given by the ODE \eqref{eq:SIR}, \code{observation} specifies that the
observed data (\code{mort}) are assumed to arise from sampling from
the normal distribution associated with ordinary least squares
(\code{ols}) fit [with a mean given by the fitted model curve,
$\gamma I(t)$, as in equation~\eqref{eq:SIR;R}], the initial
conditions are expressed as proportions of the total population size
(\code{N}), which is fixed at the estimated population of Bombay in
1906 ($10^6$), the parameters to be fitted are $\beta$, $\gamma$, and
initial proportions $s_0$ and $i_0$, which are to be fitted on the
logit scale since they are proportions (log links are assumed by
default, so $\beta$ and $\gamma$ are constrained to be positive).

\thickredline

Searching for starting values through trial and error:
<<sirmortstart,warning=FALSE,basefig=TRUE>>=
SIR_start <- c(beta=0.8, gamma=0.4, N=6000, i0=0.001)

ss_SIR <- simulate(SIR_model,
    parms = SIR_start, times = bombay$week)

plot(bombay)
lines(ss_SIR$times, ss_SIR$I)
@
Fit:
