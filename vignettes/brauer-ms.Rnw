...
models \cite{Earn+00,HeEarn07,HeEarn16,PapsEarn19}.  We hope the
...
or mechanistically.  It is worth noting, however, that a
...
\cite{Rosa+21}; a good fit is not sufficient to conclude that a model
is good.
...
\djde{perhaps mention the help command that users should use to learn
how to take advantage of these options}
\begin{description}
\item[\term{importance sampling}]\code{fitode} provides an option to
  compute confidence intervals using \needref. In this approach,
  ...
  the Delta method, but is computationally more costly.
\item[multiple data streams]\code{fitode} allows for simultaneous
  fitting of multiple state variables to multiple time series.
\item[Bayesian inference via MCMC]Markov chain Monte Carlo.
\end{description}
...
critical for estimating uncertainties in parameter estimates and the
dynamics of a system.  Popular \code{R} packages that can fit models
with process error include \code{pomp} \citep{king2015statistical} and
...
Fred Brauer was curious not only to understand how those of us who
spend time fitting dynamical models to data go about it, but why it is
hard.

... uncertainty is key to convey in applications... yet estimating the
uncertainty in our parameter estimates and forecasts is extremely
challenging...
