...
intended to submit this paper to a volume in honour of Fred's memory
\cite{BrauerMemorialIssue} \bmb{will probably need to cite this 'by hand' [Journal/volume/issue]. Leaving as ``undefined citation'' for now. FWIW footnotes [https://www.springer.com/journal/11538/submission-guidelines]{are allowed in BMB}}}
...
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July
  ...
  estimates are given in \cref{tab:Bombay}.  The dotted yellow
  curve shows the \code{fitode} fit, for which the associated
  parameter estimates are given \cref{tab:bombay.fitode}.
  \emph{Bottom panel:} The yellow curve is identical to the yellow
  dotted curve in the top panel, and the yellow band shows the
  \code{fitode} confidence band; this confidence band is shown as
  a linear interpolation between successive observation times because
  the model fits to incidence at discrete time points rather than to
  a continuous curve of the instantaneous death rate.
  The dotted red curve is identical to
