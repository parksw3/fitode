...
\swp{Some issues with parameter identifiability for Bombay. Let's fix gamma for now. Also eaier to compare.}
...
<<loglik-sir>>==
logLik(SIR_fit) - logLik(SIR_fit2)
@
...
coef(SIR_fit2)
@
Issues with parameter identifiability:
<<bombay_identifiable, cache=TRUE>>=
@
...
<<bombay_likelihood_fit, cache=TRUE, warning=FALSE, message=FALSE>>==
...
<<bombay_likelihood_plot, cache=TRUE>>==
...
<<philanlsR0>>==
cc.phila.delta <- KMcifun(phila.nlsfit, I0=I0.phila.fitode, N=philapop1918)
...
R0.phila.nls <- Reff.phila.nls/S0.phila.nls*philapop1918
@
...
\begin{table}
  \begin{center}
    \caption{Parameters of \code{fitode} fits of the SIR model \eqref{eq:SIR} to a stochastic SIR simulation.
...
\begin{figure}
<<plot_stochastic_SIR, echo=FALSE>>=
...
lines(sir.det$tau, 1 * sir.det$I * N/7, col=my.red, lwd=lwd) ## dR/dt
...
legend("topright", bty="n", lwd=c(2,lwd,lwd,lwd*0.60),
       ...
       legend=c("observed data","KM","deterministic","fitode (nbinom)"))
@
...
\begin{table}
  \begin{center}
    \caption{Parameters of \code{fitode} fits of the SIR model \eqref{eq:SIR} to Philadelphia flu epidemic.
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates for the Philadelphia epidemic
        of \KM's \cite{KermMcKe27} approximation
      \eqref{eq:sech} to the SIR model \eqref{eq:SIR}
      using nonlinear least squares
      (\code{nls}).
      The $\R_0$ estimates assume the population of Philadelphia
      in 1918 is....
}\label{tab:philanls}
    \medskip
    %% making use of ragged2e and array packages:
