...
package \cite{adaptivetau}.  The demographic stochasticity (aka
\term{process noise}) that these algorithms simulate is essential to
capture real effects that occur when the number of infected
individuals is small (especially the possibility that an epidemic can
burn out \cite{Pars+24}).
