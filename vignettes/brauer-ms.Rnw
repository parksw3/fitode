...
to dedicate this tutorial to his memory.\footnote{We had hoped to
  submit this paper to a volume in honour of Fred's memory
  \cite{BrauerMemorialIssue}, but didn't complete the manuscript in
  time.}

\djde{Humph.  We'll have to say something different.}  \bmb{Should we
  still use a Brauer-centric framing since the paper is no longer
  going to that special issue? I don't object to it but thought we
  should at least consider (if we do use it, we should probably
  mention somewhere that the paper was originally intended for a
  Brauer memorial issue)} \djde{What do you think of my revision?  It
  would make me feel good to mention the history with Fred.  I'm not
  convinced that highlighting my failure to meet the deadline for the
  memorial issue is necessary, especially given that we don't plan to
  submit to JBD. Perhaps you can improve on the language?}
...
\cref{eq:sechparams} can be inverted
...
\djde{I reorganized this paragraph a bit, and corrected
  some typos in equations.}
...
\djde{There was commented-out text at this point, which I've converted
  to grey.  If we definitely don't want it, it should be moved to
  \texttt{cuts.Rnw}.}  {\color{gray} where, as for the likelihood
  associated with Normal errors, we assume $\sigma^2$ is computed as
  the variance of the residuals across the full time series.\djde{Is
    that what's done, or is $\sigma^2$ added to $\thetavec$ as a
    parameter to fit?  or even $\sigma_i^2$ for each i?}  \swp{It is
    what's done. There's a version where $\sigma^2$ is added to
    $\thetavec$ but it is not used when we call the ols
    function. $\sigma_i^2$ doesn't make sense because we have one
    residual per $i$.}  }
...
parameters are extreme, with an $\mathcal R_0$ of 2310
(\cref{tab:philanls}).  \swp{Again, possibly something wrong with
  parameter conversions here too.  Haven't been able to figure out how
  to do it properly.}  The SIR model gives a visually similar fit as
...
the size and timing of the peak.  \swp{Again, possibly something
  wrong.}
...
The approximation predicts a much lower epidemic peak and a shorter duration of the epidemic (\cref{fig:stoch}, top panel, blue curve).  \swp{Parameter values are a little screwed up at the moment. Doesn't seem like it's a trivial fix.}  Nonetheless, we can still fit the KM approximation to the simulated epidemic and obtain visually reasonable agreement (\cref{fig:stoch}, bottom panel, red curve; \cref{tab:stoch.nls}); the fit cannot get better because the approximation \eqref{eq:sech} is symmetric about the peak, whereas the rise is steeper than the fall in the true solution.  \swp{we don't want to interpret these parameter estimates as being ``wrong'' until we figure out the parameter conversion issues.}
...
\swp{I feel we need a way to distinguish nls KM approx vs parameter matching KM approx}
\begin{itemize}
    \item We present a guideline/tutorial for fitting ODE models
    to epidemic time series data alongside a user-friendly \code{R}
    package for beginners.
    \item In doing so, we compare SIR model fits to KM's approximations
    across three data sets and their resulting parameter estimates.
    \item For the Bombay plague data, both models give similar fits and parameter
    estimates.
    \item For the Philadelphia flu data, nonlinear least square fits of KM's approximation
    give extreme parameters even though it provide a visually good fit.
    \item Finally, fitting a deterministic SIR model to a stochastic simulation shows that
    underlying epidemiological parameters, except for the initial conditions, can be recovered
    with reasonable accuracy.
\end{itemize}

\begin{itemize}
    \item Despite historical connections to KM, it is unclear whether
    the SIR model is appropriate for studying the Bombay plague
    data.
    \item Instead, Baca\"er argued that the epidemic represents a
    seasonal outbreak and therefore requires a change in transmission
    rate over the course of an epidemic.
    \item Here, we find that there are issues with parameter identifiability
\end{itemize}

\begin{itemize}
    \item Features implemented in fitode but not covered in this paper
    \item Calculating confidence intervals using importance sampling.
    Expected to be more robust than the Delta Method.
    \item Simultaneous fitting to multiple time series (e.g., predator-prey dynamics)
    \item MCMC \& Priors
\end{itemize}

\begin{itemize}
    \item There are many other issues that are not covered
    by the \code{fitode} package that are important/useful for making
    statistical inference from epidemic time series.
    \item Flexible variation in transmission rate. TSIR \citep{grenfell2002dynamics,becker2017tsir}.
    \item Process error. Many packages that deal with this: pomp \citep{king2015statistical},
    odin \citep{odin2021}.
    \item Agent based models. Covasim \citep{kerr2021covasim}
    \item Fixed and random effects on parameters
\end{itemize}
