...
and the observed data).  Then the joint probability 
density $\pdens$ of the data given the
model is
%%
...
(the elements $\Delta x_{\tindex}$ will always appear as a constant multipliers
or divisors and don't affect any of our conclusions). 
