...
\noindent
which would prevent convergence to negative $\omega$ and
$\phi$. \swp{Is this a good place to introduce link functions? Or too
  early? Or at least hint at it?  Example text:} \swp{Instead, we can
  also estimate the parameters on the log scale and convert them back
  to the the original scale by exponentiating the estimated
  ...
  I'll revisit this when I've got further with writing the rest of the
  paper.}  In general, the potential existence of multiple local
optima is something that makes fitting models to data hard, and you
need to be cautious in interpretting any fit to which your fitting
