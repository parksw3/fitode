...
\djde{Notation: I've introduced macros \macro{Sinit}, \macro{Iinit},
  \macro{Rinit}, which are currently set to $\Sinit,\Iinit,\Rinit$.
  We can discuss whether this is good.  I also defined \macro{Rn}
  for $\Rn$, to reduce potential confusion with \macro{Rinit}.
  Hopefully I got this right\dots}
\bmb{the Roman ``i'' in $\Sinit$ etc. does look a little odd, but I see the point. It might (?) be worth a short remark the first time it's used to emphasize that we're using it to differentiate (especially) $\Rinit$ from $\Rn$}
...
\djde{The issue is that we follow \KM and present a formula for
  $\dbyd{R}{t}$ rather than $I(t)$.  In practice, it is the prevalence
  curve that people care about.  hmmm.}
...
\end{equation}
\end{linenomath*}
where $\Rinit=R(0)$ (\KM assumed $\Rinit=0$); thus,
...
\djde{We could add that this will be true initially for any $\Rn$,
  and true for all time if $\Rn$ is sufficiently close to 1, which
...
Choosing this objective function is equivalent to assuming that the $\traj[\ti]$ values are direct (but noisy) observations of the state variable $\traj(t)$. When the connection between the dynamical system and our observations is more complicated, we need to define an explicit \term{observation process}; see \cref{sec:Uncertainty}.
Minimizing
...
probability of observing the data $\{\traj[\ti]\}$ given $\{\traj(\ti)\}$.
We imagine that the
...
model is
\djde{$\le$ instead of $<$ on the left), and reformatted a bit.}
\bmb{Some issues addressed/deleted. I guess $\le$ is OK.  In the continuous limit it doesn't matter \ldots}
%%
\begin{linenomath*}
  \begin{equation}\label{eq:prob.data.given.model}
    \begin{split}
      \Pop(\text{data} \mid \text{model})
      & = \lim_{\Delta x \to 0} \frac{\Pop\big(\traj(\ti;\theta) \le \traj[\ti] < \traj(\ti;\theta) + \Delta x \big)}{\Delta x} \\
      & = \prod_{\tindex=1}^n
\left[\frac{1}{\sqrt{2\pi\sigma^2}}
\exp\left(-\frac{\big(\traj(\ti;\thetavec) - \traj[\ti]\big)^2}{2\sigma^2} \right)
\right]
\,.
\end{split}
\end{equation}
\end{linenomath*}
%%
\djde{The reason for writing $\Delta x[\ti]$ previously was to allow
  for unevenly spaced time points.  It's obviously cleaner the above way.
  Not sure if this simplification issue is worth mentioning.  We could
  say ``(assuming for simplicity that the observation times are evenly
  spaced)'' or something like that.}
...
\bmb{There's a subtle point we might want to mention here, which is that we are sweeping the estimation of $\sigma$ under the rug; everything that's here is mathematically correct (we don't have to worry about constant scaling factors when we are only looking for arg min) \ldots Thinking about whether there is a short/simple/clear comment we can make to reflect that we are not including $\sigma$ in the set of estimated parameters here}
...
$g(\thetavechat)=(\hat{\Reff}, \hat{\gamma},\hat{\Sinit})$.  This is
...
\caption{\textbf{The plague epidemic in Bombay}, 17 December 1905 to 21 July
  ...
  coincide almost exactly with KM's fit \bmb{check this - I think it's a coincidence, and doesn't look exactly identical, but is close enough to make me nervous!}.
  %%
