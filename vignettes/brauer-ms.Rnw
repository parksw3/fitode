...
%%
  \begin{equation}
    \gradtheta\log\lik\big(\xvec(t,\thetavec)\big)
    = \frac{1}{\lik\big(\xvec(t,\thetavec)\big)}
      \gradx\lik\big(\xvec(t,\thetavec)\big) \, \sensmat(t) \,.
  \end{equation}
%%
...
\item[\code{fixed}]specifies parameter values to be fixed (and
  ...
  unidentifiability\footnote{In short, unidentifiability of $\gamma$
    ...
    of $\gamma$. \swp{While we can still fit the model without fixing $\gamma$,
    resulting estimates become too sensitive to starting conditions and
    numerically unstable, which can prevent us from obtaining the Hessian matrix
    and therefore calculating confidence intervals. It would be also possible to
    fit the model by fixing other parameters and estimate $\gamma$ instead. We chose
    to fix $\gamma$ as the duration of infection can be often estimated from
    independent data sources (and to remain consistent with our \code{nls} fits
    of the KM approximation).}}.
