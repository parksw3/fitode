...
Our data are actually non-negative, discrete counts of deaths (or cases in other contexts),
so the observation error distribution can't really follow a continuous, real-valued Normal distribution
(in our previous example, the assumption of normality is allowing the lower limit of the
confidence band on the fitted model to drop below zero when the number of deaths is small).  We could assume a Poisson error distribution
with mean equal to the fitted model trajectory
[equation~\eqref{eq:SIR;R}], but typical observations have variance
greater than the mean (the variance is equal to the mean for a Poisson distribution).
