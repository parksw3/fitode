...
\swp{I don't think we need this demonstration.}
<<percent-diff>>=
percent.difference <-
...
\bmb{can we come up with a similar geometric intuition for $\omega$?
  Wild guess of 1 is OK here and will be OK \emph{when parameters have
    a reasonable scaling}, but \ldots} \djde{Assuming $I_0/S_0\ll1$,
  $\omega\sim1/2$ the initial growth rate, so it can be approximated
  easily by plotting the data on a log scale and estimating the slope
  (and dividing by 2).  That's not as trivial as guessing $a$ and
  $\tpeak$ from looking at the time series, but does seem preferable
  to a wild guess.}
...
\swp{I don't think it's worth pursuing KM approx any further... we're
  trying to show how to fit an ODE, not KM approximation.}\djde{That's
  not my perspective.  We're trying to show how to fit a model to
  data for people who haven't a clue, and I think it is nice to
  show how easy it would have been for KM to do this right.}
\swp{OK. That seems fine. But we should divide up the equation below to say
that we want to integral. And then say that for KM, integral of dR/dt is so and so.}

Something we have glossed over is that approximating weekly mortality
(what is observed) by $\dbydt{R}$ is unnecessary.  A better estimate
of weekly mortality reported at time $t_i$ would be to take
\begin{linenomath*}
  \begin{align}\label{eq:sechcorrect}
f(t_i;\thetavec) &=
\int_{t_{i-1}}^{t_i} \ddt{R}\,\dt \\ \label{eq:correct.mortality}
&= \int_{t_{i-1}}^{t_i}  a \sech^2{(\omega\,t - \phi)}\,\dt \\
&= \frac{a}{\omega}\Big(\tanh{(\omega\,t_i - \phi)}
  - \tanh{(\omega\,t_{i-1} - \phi)}\Big) \,.
\end{align}
\end{linenomath*}
We have not done this because \KM didn't, \djde{Did Baca\"er
  \cite{bacaermodel2012} discuss this?}\swp{no}
  and we wanted to compare our
fits to theirs.  However, there is no question that we should really
be fitting to weekly aggregations, and we do this below when fitting
to the ODEs directly.
...
numerical solution of an ODE; we must simultaneously solve a set of \term{sensitivity equations} alongside the main differential equations
...
The \code{fitode} package\footnote{\code{fitode} is available on CRAN
  ...
  \code{install.packages("fitode")}.} does all of this computational work
under the hood, and makes it as easy for a user to fit an ODE to data
as it was for us to use \code{nls} above to fit a curve based on an analytical formula.
...
\bmb{I think it would be nice to compare, but we might be able to minimize the actual amount of code (e.g. say in passing ``OLS was fitted by using \code{ols} as the response distribution'', although that would miss the explanation of profiling out $\sigma$ \ldots)}
\djde{We can just give the nbinom code but say we also fitted
  with \code{ols}, i.e., changing the {\tt\hlkwc{observation}}
  list to: {\tt
%%\begin{alltt}
  \hlstd{mort}
  %%\hlopt{~} %% FIX: this comes out blank, hence:
  {\color{black}\~{}}
  \hlkwd{ols}\hlstd{(}\hlkwc{mean} \hlstd{=
    gamma} \hlopt{*} \hlstd{I)}
%%\end{alltt}
}
}
\swp{Rewrote some parts here to reflect nbinom implementation + correct mortality handling}
\item[\code{observation}]specifies that the observed data
  (\code{mort}) are assumed to arise from sampling from the negative binomial
  distribution with a dispersion parameter of $k$ (\code{dnbinom}); the
  mean of the distribution is given by the fitted model trajectory [equation~\eqref{eq:correct.mortality}]
\begin{linenomath*}
\begin{equation}
  \mu(t) = \int_{t_{i-1}}^{t_i} \ddt{R}\,\dt
\end{equation}
\end{linenomath*}
which is equal to $R(t_i) - R(t_{i-1})$. This is implemented by using the
\code{diffnames} argument to specify the state variable, for which consecutive differences
need to be calculated and returned (instead of its raw value). Ordinary least squares
can be implemented by simply changing the \code{observation} to \code{ols(mean = R)}
...
\end{description}
We take our previous parameter estimates from \code{nls} as our starting values (we first plot the results of integrating the ODEs for these parameters to make sure they give a reasonable solution).
\swp{Note that we still have to assign a parameter value for $k$ even though it is not used for simulating the SIR model.}
...
@
We already have a pretty good fit. Note that \code{fitode} returns \code{NA} at
$t=1$ because we are starting from $t=1$, meaning that $R(2) - R(1)$ is the first
value we can calculate. So we have to add an extra column to the data set when we're
actually doing the fit:
...
<<sirmortfitcoef>>==
bombay.fitode.nb.coef <- coef(SIR_fit)
bombay.fitode.nb.ci <- confint(SIR_fit)[,-1]
bombay.fitode.nb.beta <- bombay.fitode.nb.coef[1]
bombay.fitode.nb.gamma <- bombay.fitode.nb.coef[2]
bombay.fitode.nb.S0 <- bombay.fitode.nb.coef[3]
bombay.fitode.nb.I0 <- bombay.fitode.nb.coef[4]
bombay.fitode.nb.k <- bombay.fitode.nb.coef[5]
@
...
@
The fit is very good, but the estimated parameters are unrealistic, with a mean infectious period ($1/\gamma$) of 0.3 days.
<<sirmortcoef>>=
coef(SIR_fit)
@
What happens if we force the infectious period to be more realistic (longer: i.e., fix a smaller value of $\gamma$)?
...
Plot? \swp{This is broken because fitode is having trouble calculating vcov... probably because there are issues with
identifiability and the curvature is weird}
<<plot-sir-fit2, eval=FALSE>>==
...
The fitted curve still looks good, but now the estimated effective
population size \bmb{do we use this terminology? should we?} $S(0)$ is unrealistically small.
\swp{I don't like this term. We should just call it susceptible population size or something...}

\bmb{There's some interesting stuff here (non-identifiability/sensitivity/possible inappropriateness of simple SIR for Bombay) but needs more integration/discussion?}
\swp{We noticed non-identifiability back when we were working on fitsir. We could spend more time talking about this and show likelihood surfaces? or not?}

\djde{moved KM weekly mortality comment up}

<<sirmortmodel, echo=FALSE>>=
...
<<sirmortfit_ols_confband, eval=FALSE>>==
SIR_ols_confband <- predict(SIR_fit_ols, level=0.95)$mort
@
...
sir.stoch.data <- data.frame(
    time=0:10,
    mort=c(NA, diff(sir.stoch$R[!duplicated(ceiling(sir.stoch$time),
                                            fromLast = TRUE)]))
)
...
\begin{table}
  \begin{center}
    \caption{Parameters of \code{fitode} fits of the SIR model \eqref{eq:SIR} to Bombay data.
      The $\R_0$ estimates assume the population of Bombay was
      $N=1$ million \cite[p.\,408]{bacaermodel2012}.
  ...
  \begin{tabular}{ m{3cm} | c | c | c c | c c}
    ...
    transmission rate & $\beta$ & $\frac{1}{\textrm{weeks}}$ &
      \Sexpr{bombay.fitode.nb.beta} & \Sexpr{ci_fmt("beta", bombay.fitode.nb.ci)}  & ?? & ?? \\
    removal rate & $\gamma$ & $\frac{1}{\textrm{weeks}}$ &
      \Sexpr{bombay.fitode.nb.gamma} & \Sexpr{ci_fmt("gamma", bombay.fitode.nb.ci)}  & ?? & ?? \\
    initial\break susceptibles & $S_0$& -- &
      \Sexpr{as.integer(bombay.fitode.nb.S0)} & \Sexpr{ci_fmt("S0", bombay.fitode.nb.ci, max_ord = 6)} & ?? & ?? \\
      initial prevalence & $I_0$ & -- &
      \Sexpr{bombay.fitode.nb.I0} & \Sexpr{ci_fmt("I0", bombay.fitode.nb.ci)} & ?? & ?? \\
      overdispersion\break parameter & $k$ & -- &
      \Sexpr{bombay.fitode.nb.k} & \Sexpr{ci_fmt("k", bombay.fitode.nb.ci)} & ?? & ?? \\
  \end{tabular}
  \end{center}
\end{table}
\end{landscape}
