...
\sum_{\tindex=1}^n \big(\traj(\ti;\thetavec) - y_\tindex\big)^2 \,,
\end{equation}
\end{linenomath*}
where the observed data are the points $\{(\ti,y_\tindex):\tindex=1,\ldots,n\}$,
...
fitted values and illustrates a very important fact: there is
\emph{not necessarily a unique best fit set of parameters!}  In this
...
probability of observing the data $\{y_\tindex\}$.  We imagine that the
...
\exp\left(-\frac{\big(\traj(\ti;\thetavec) - y_\tindex\big)^2}{2\sigma^2} \right) \Delta{y_\tindex}
\right]
\,.
\end{equation}
\end{linenomath*}
...
\;+\; \text{constant}
\Big) \\
&= \argmin_\thetavec\sum_{\tindex=1}^n \big(\traj(\ti;\thetavec) - y_\tindex\big)^2
 \,,
\end{align}
\end{subequations}
\end{linenomath*}
...
\end{equation}
\end{linenomath*}
The predicted variance of a particular observation $y_\tindex$ is given by
$\mu_i (1+\mu_i/k)$, where $\mu_i(\thetavec) = \traj(\ti;\thetavec)$ is the
...
\begin{split}
 \thetavechat
 &= \argmin_\thetavec\sum_{\tindex=1}^n \Big(\!\!
                -\log\Gamma\big(y_\tindex+k\big)
                +\log\Gamma(k) + \log(y_\tindex!) \\
                &\hspace{3cm}
                -k \log\left(\frac{k}{k+\mu_i(\thetavec)}\right)
                -y_\tindex \log\left(\frac{\mu_i(\thetavec)}{k+\mu_i(\thetavec)}\right)
                \Big)
  \,.
\end{split}
  \end{equation}
