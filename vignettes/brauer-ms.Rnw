...
\sum_{i=1}^n \big(\traj(t_i;\thetavec) - y_i\big)^2 \,,
\end{equation}
\end{linenomath*}
where the observed data are the points $\{(t_i,y_i):i=1,\ldots,n\}$,
$\traj(t;\thetavec)$ is the model \eqref{eq:sech}, and the parameter
...
\exp\left(-\frac{\big(\traj(t_i;\thetavec) - y_i\big)^2}{2\sigma^2} \right) \Delta{y_i}
\right]
\,.
\end{equation}
\end{linenomath*}
...
\;+\; \text{constant}
\Big) \\
&= \argmin_\thetavec\sum_{i=1}^n \big(\traj(t_i;\thetavec) - y_i\big)^2
 \,,
\end{align}
\end{subequations}
\end{linenomath*}
...
$\mu_i (1+\mu_i/k)$, where $\mu_i(\thetavec) = \traj(t_i;\thetavec)$ is the
...
considering $g(\thetavec)=\traj(t_i;\thetavec)$.  This is how we obtained
...
the log-likelihood of the observations at times
$\{t_1,t_2,\ldots,t_{n_t}\}$ with respect to the parameters $\thetavec$,
%%
\begin{linenomath*}
\begin{subequations}\label{eq:2nd.chain}
\begin{align}
  \gradtheta\log\lik(\thetavec)
  &= \gradtheta\log\Pop\big(\{\xvec_{t_i}:i=1,\dots,n_t\} \mid \xvec(t,\thetavec)\big) \\
  &= \gradtheta\log\prod_{i=1}^{n_t} \Pop\big(\xvec_{t_i} \mid \xvec(t,\thetavec)\big) \\
  &= \sum_{i=1}^{n_t} \gradtheta\log \Pop\big(\xvec_{t_i} \mid \xvec(t,\thetavec)\big) \\
  &= \sum_{i=1}^{n_t} \frac{1}{\Pop\big(\xvec_{t_i} \mid \xvec(t,\thetavec)\big)}
     \gradx\log \Pop\big(\xvec_{t_i} \mid \xvec(t,\thetavec)\big) \, \sensmat(t) \,,
\end{align}
\end{subequations}
\end{linenomath*}
%%
...
\begin{equation}
\Pop\big(\xvec_{t_i} \mid \xvec(t,\thetavec)\big) =
\prod_{j=1}^n\texttt{NB}(x_{j,t_i};x_j(t_i,\thetavec),k) \,.
\end{equation}
...
\item[\code{fixed}]specifies parameter values to be fixed (and
  ...
  by fixing a different parameter instead and estimating $\gamma$.  We
  typically choose to fix $\gamma$ because the mean duration of
  infection ($1/\gamma$) can be often estimated from independent data
  sources; here, to make comparisons of fits easier to interpret, we
  have fixed $\gamma$ to the value we estimated via \code{nls} fits of
  the \KM approximation \eqref{eq:sech}.}.
...
SIR_ols_confband <- predict(SIR_fit_ols, times=tvals)$mort
@
...
\begin{table}
  \begin{center}
    \caption{Parameters of the SIR model \eqref{eq:SIR}, as estimated
      ...
      parameter unidentifiability. $\gamma$ was fixed to
...
lines(estimate ~ times, data=SIR_ols_confband, col="purple", lwd=lwd*0.60)
points(mort ~ week, data = fitode::bombay, xpd=NA, pch=21, bg="white", lwd=2)
legend("topleft", bty="n", lwd=c(2,lwd*0.60,lwd,lwd*0.6),
       lty=c(NA,"dotted","solid","solid"),
       col=c("black",col.nls,col.fitode.nb,"purple"),
       pch=c(21,NA,NA,NA),
       pt.bg=c("white",NA,NA,NA),
       legend=c("observed data","our nls fit","fitode (nbinom)", "fitode (ols)"))
@
\caption{The plague epidemic in Bombay, 17 December 1905 to 21 July
  ...
  dotted curve in the top panel
    , and the yellow band shows the
  ...
  panel. The purple curve shows the \code{fitode} fit by minimizing
  the ordinary least squares.
  We have separated the two panels because the confidence band
  overlap would make the plots difficult to interpret. }
\label{fig:Bombay}
\end{figure}
...
\begin{table}
  \begin{center}
    \caption{Parameter estimates for the 1918 influenza epidemic in
      ...
      \cref{tab:phila.fitode}).
      Instead
      of using the raw population size to estimate $\mathcal R_0$,
      we consider the size of population that would eventually die
      if they were infected, which was calculated by multiplying
      the population size of Philadelphia
      (\Sexpr{prettyNum(philapop1918, big.mark=",")}) with the assumed CFP of 0.025.
      \swp{See if this is appropriate and edit accordingly.}
...
\caption{The main wave of the 1918 influenza epidemic in the city of
  ...
  \swp{I thought we didn't want to use the same colors for different fits...
  we're already using yellow dotted in Figure 1 for something else? but your call...
  I don't object.}
}
\label{fig:phila}
\end{figure}
...
\begin{table}
  \begin{center}
    ...
    \djde{I don't understand that.  What does the KM column
      ...
      \swp{These are also in KM because we converted from true to KM. So KM to derived parameter
      recovers the true derived parameter.}
}\label{tab:stoch.nls}
    \medskip
    %% making use of ragged2e and array packages:
    \RaggedRight
...
\caption{Deterministic fits to daily incidence generated by a
  ...
  mean generation interval $\Tg=1$ week.  \djde{Daniel, did you
    ...
    day?} \swp{Just counted the number of new recovery (death) each day.}
  %%
