...
\bmb{I would normally object to ``exponentially'' on pedantic grounds but here it's actually justified \ldots}
...
$t$.''  Since (non-human\footnote{\url{https://en.wikipedia.org/wiki/Computer_(occupation)} \bmb{will remove this footnote eventually}}) computers did not yet exist, and an exact analytical
...
parameters needs to be specified separately, and we have taken this to
be the initial prevalence $I_0$.  From
...
The final column of Table~\ref{tab:Bombay} lists 95\% confidence
intervals on our \code{nls} parameter estimates.
