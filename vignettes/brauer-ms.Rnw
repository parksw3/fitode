...
\term{agent-based models}\needref, which represent each individual as
a separate unit that can have unique properties.
...
equation~\eqref{eq:sechparams} cannot be inverted analytically, so we
...
a_fun <- function(Reff, gamma, S0, I0)
    {(S0/Reff)*omega_fun(Reff,gamma,S0,I0)}
...
find_orig_params <- function(orig.params.guess, fitted.params) {
    ...
    return(c(out, S0=S0, beta=out[["Reff"]]*out[["gamma"]]/S0,
             Tg=1/out[["gamma"]]))
}
Npop <- 10^6 # Bombay population in 1900
S0 <- Npop - 1
(orig.params.guess <- c(Reff=2, gamma=0.2, I0=Npop-S0))
