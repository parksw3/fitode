...
\bmb{For future reference: ORCID 0000-0002-2127-0443, twitter @bolkerb, Mastodon @bbolker@fediscience.org}

\swp{Mine: ORCID 0000-0003-2202-3361, twitter @sang\_woo\_park, Mastodon @sangwoopark@ecoevo.social}
...
$t$.''  Since Since (non-human\footnote{\url{https://en.wikipedia.org/wiki/Computer_(occupation)}
\bmb{will remove this footnote eventually}}) computers did not yet exist, and an exact analytical
did not yet exist, and an exact analytical
...
close to the observed data points.  The most obvious
...
\bmb{the \code{(...)} trick for printing the value of an assignment
  is nice and compact, but might be confusing for naive readers? (i.e.
  do we want an explicit \code{print()} statement here, and throughout?)}
...
but in general, there can be multiple local minima that cause \code{nls}
to converge to points that may or may not represent equally good fits to the data.  In general, the potential existence of multiple local optima is something
that makes fitting models to data hard; you need to be cautious in
interpreting any fit to which your fitting algorithm has converged.
\bmb{\cite{raue2013lessons} have some suggestions about how to diagnose
  multiple optima but this is probably beyond the scope of this paper}
...
trick, but there is more to it than that.  A much broader
range of optimization algorithms is available for unconstrained fitting;
numerical parameter values of very small magnitude can also
lead to numerical instability,
so it is advantageous to link parameters that must lie in a given range to
unconstrained parameters that can be fit more easily.  In
...
probabilities.
%%
\swp{Maybe add this so it doesn't look like we're introducing this for
no reason: Link functions are implemented in \code{fitode} as we
demonstrate later.}
\djde{Good idea.
  I've added some text along these lines.  I'll revisit this when I've
  got further with writing the rest of the paper.}
  \swp{I still wonder if it's a little early to introduce link functions if we're
not going to use them until much later when we actually use fitode. Or maybe
the last sentence I suggested helps with that issue a bit.}
