...
chain rule, i.e. if we write
\begin{equation}
S(x,t, \theta_i) = \frac{\rm d}{{\rm d}t}\left(\frac{\partial
...
\end{equation}
then we can calculate the derivative of $x(t)$ with respect to $\theta_i$ as $\int_0^{t} S(x,t') \, dt'$, i.e. by solving the differential equation; we can then use a further chain-rule step to compute the derivative of the log-likelihood of the observations at time $t$ with respect to the parameters. Integrating the sensitivity equations in parallel with the main set of ODEs is
a computationally efficient and numerically stable way to
calculate the overall gradients of the log-likelihood with respect to
the parameters, which makes nonlinear estimation more robust and efficient. We can also use these gradients to calculate CIs using the Delta method.
Raue \emph{et al.} \cite{raue2013lessons} give a more detailed comparison between using the sensitivity equations and computing gradients by finite-difference approximations.
