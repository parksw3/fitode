...
SIR_model <- odemodel( 
    ...
    par = c("beta", "gamma", "N", "s0", "i0"),
    link = c(s0 = "logit", i0 = "logit")
)
@
}% end vbox
...
SIR_start <- c(beta=0.8, gamma=0.4, N=10^6, s0=0.05, i0=0.0001)
...
which \code{fixed} at the estimated population of Bombay in 1906
($10^6$)
...
SIR_fit2 <- fitode(
    ...
    data = bombay,
    start = SIR_start,
    fixed = c(N = 10^6, gamma = 1), # Tg = 1/gamma = 1 week
    tcol = "week"
)
@
