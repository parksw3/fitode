...
and review articles \cite{Heth00,Earn+02,Earn08,Earn09}.  Researchers have
primarily focused on \term{compartmental models} like the SIR model,
cast either as differential equations following the tradition of \KM
\cite{KermMcKe27}, or as \term{stochastic processes} in the tradition
of McKendrick \cite{McKe26} and Bartlett \cite{Bart60}.  In recent
years, as the power of computers has grown exponentially,
\bmb{do we need ``exponentially'' here? it is true in this case but I would still
  prefer to drop it}
there has
been increasing interest in \term{agent-based models}, which represent
each individual as a separate unit that can have unique properties
...
Fred's memory.\djde{Humph.  We'll have to say something different.}
\bmb{Should we still use a Brauer-centric framing since the paper
  is no longer going to that special issue? I don't object to it
  but thought we should at least consider (if we do use it, we
  should probably mention somewhere that the paper was originally
  intended for a Brauer memorial issue)}
...
\label{eq:correct.mortality} \\
  ...
  \end{subequations}
\end{linenomath*}
\bmb{is it worth mentioning the assumption that infection fatality rate $\approx$ 100\% again (``Note that
we are still assuming \ldots''?)}
...
\djde{There is commented-out text at this point.  I prefer deletions
  to go in a cuts file.  Leaving here for now.}
...
$\thetavechat$ ($\pm 1.96$ represents a range containing 95\% of the
probability of a standard Normal distribution) \bmb{added the
  explanation of $\pm 1.96$; I'm really unsure
  how much probability \& stats we should try to explain \ldots}.  To obtain CIs on functions of the fitted parameters
...
Better confidence intervals can be obtained
using the \term{profile likelihood}, which is calculated by fixing a
set of model parameters to specific values and fitting the remaining
parameters to maximize the likelihood \citep[][\S 7.5.1]{Bolk08}.  By calculating
the profile likelihood across a range of parameter values, we obtain
the profile likelihood surface, from which confidence intervals can be
estimated using the likelihood ratio test.\djde{we should either spell
  that out, or give a reference to a section or page number in a book
  that gives examples of getting CIs from profile likelihoods this
  way}
\bmb{added section ref to Bolker 2008 citation above}
While profile likelihoods generally give more accurate
estimates of confidence intervals, calculating the profile likelihood
can be challenging, if not practically impossible, for derived
parameters or epidemic trajectories \bmb{could ref \cite[][\S 7.5.1.2]{Bolk08} if you wanted}.  Consequently, we rely on the
Delta Method hereafter.
...
@
\bmb{is it worth a note/footnote here to caution about the common practice of fitting to cumulative curves, e.g.
  \cite{King+15}?}
