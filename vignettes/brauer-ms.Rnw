...
series is near zero).  We could assume a Poisson error
distribution\djde{with mean\dots}, but typical observations have
variance greater than the mean (which are equal for a Poisson
distribution).  The negative binomial distribution can be considered
an overdispersed Poisson, which makes it a good choice.  \djde{Ben,
  please feel free to give a more principled justification, and to
  explain how the neg binom dispersion parameter would enter the
  likelihood function if we wanted to fit it.}
...
\djde{mentioning profile likelihoods in passing, with a reference, 
  would be good; we could add that here}
...
\djde{moved from above: this comment from Daniel needs to go in this section:}
...
\djde{We will need to explain what the sensitivity equations are.}
