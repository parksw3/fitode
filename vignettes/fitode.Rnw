...
<<opts,echo=FALSE>>=
library("knitr")
## DON'T cache, suppress warnings globally
...
This can be done using the \code{update} function:
<<nbupdate>>=
...
Alternatively, we can update \code{odemodel} objects and refit the model:
<<nbupdate2>>=
exp_model_nbinom <-
  update(exp_model,
...
## FIXME: we need this now because I'm checking links more carefully
