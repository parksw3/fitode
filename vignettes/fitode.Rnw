...
Using `confint()` on the two models shows that the confidence intervals on the parameters
are nearly identical, except for the two negative binomial parameters which have extremely
wide (even ridiculously wide) confidence intervals; the standard Wald approximation fails here.
