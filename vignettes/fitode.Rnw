...
\newcommand{\needref}{{\color{orange}(REFERENCE PLEASE)}}
...
\code{fitode} is an R package for fitting ordinary differential
equations (ODE) using Maximum Likelihood \needref or Bayesian Markov
Chain Monte Carlo (MCMC) \needref. It relies on symbolic
differentiation features of the \code{Deriv} package \citep{DerivPkg}
to solve the sensitivity equations \needref so that gradient-based
optimization algorithms \needref can be used.
\djde{What is a good pedagogical intro source that defines all the
  jargon you use below?  I've added one for the Delta method.}
\begin{itemize}
    ...
    \item confidence intervals on parameters and their transformations via
            delta method \citep{VerH12}, profiling, and importance sampling
\end{itemize}
...
as stochastic simulated observations based on the ODE
solution\djde{meaning what exactly? ``based on the ODE'' could mean
...
<<cisirb>>=
## temporarily commented out; Hessian on original scale not working?
