...
starting from zero recovered individuals, then $S0 \approx N$. As we all know $\rzero$ is monotonically related to final size *as a proportion of the
...
sir.pred <- function(p) {
    ...
    I0 <- exp(p[["logI0"]])
    S0 <- exp(p[["logS0"]])
    ode.params <- c(p[c("logR0m1", "logr")], N = S0)
...
```{r human2}
trfun <- function(logr, logR0m1, logI0, logk) {
    c(beta = exp(logr)*(1+1/exp(logR0m1))/bombay_pop,
      gamma = exp(logr - logR0m1),
      S0 = bombay_pop - exp(logI0),
      I0 = exp(logI0),
      k = exp(logk))
}
trmat <- apply(parmat, 1, \(x) do.call(trfun, as.list(x)))
cbind(est = do.call(trfun, as.list(opt1$par)),
...
What if we do what's done in the paper and fix $I(0)=1$ and $\gamma=4.47$?
...
SIR_start1 <- with(as.list(opt1$par),
                  c(beta = exp(logr)*(1+1/exp(logR0m1))/bombay_pop,
                    gamma = exp(logr - logR0m1),
                    S0 = bombay_pop - exp(logI0),
                    I0 = exp(logI0),
...
* work back toward doing this with `fitode`
   ...
   * (possibly problem in fitode of parameter scaling - even though we have derivatives, and `numDeriv::jacobian()` uses Richardson extrapolation, could still be screwed if the *initial* epsilon is bad ... ?)
* estimates of $\rzero$ are **extremely** small - does this match with Bacaer, K&M, etc. ? (Is this where we came in?)
